...
조금 다르게 처리해야 합니다.
보안상의 이유로, 포크 PR에서는 `BENCHER_API_KEY`과 `GITHUB_TOKEN`과 같은 비밀이 GitHub Actions에서 사용할 수 없습니다.
...
여기에는 `BENCHER_API_KEY`과 `GITHUB_TOKEN` 같은 비밀은 사용할 수 없습니다.
...
`BENCHER_API_KEY`과 `GITHUB_TOKEN` 같은 비밀이 사용 가능하기 때문입니다.
