...
3. [使用するブランチを選択](http://localhost:3000/docs/ja/explanation/branch-selection)します:
        1. 既に存在する場合は、現在のブランチのデータを使用します。(例： `--if-branch '${{ github.head_ref }}'`)
        2. 既に存在する場合は、PRターゲットブランチのデータと閾値のクローンを作成します。(例： `--else-if-branch '${{ github.base_ref }}'`)
...
5. `--ci-*` フラグでプルリクエストのコメントを設定するすべての方法については、[bencher run](/docs/ja/explanation/bencher-run)ドキュメンテーションを全て覧下さい。
