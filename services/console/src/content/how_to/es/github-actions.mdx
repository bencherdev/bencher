...
8. Haz una copia de tu código fuente. (ej: `uses: actions/checkout@v4`)
...
3. Establece [los permisos para el `GITHUB_TOKEN`](https://docs.github.com/es/actions/using-jobs/assigning-permissions-to-jobs#setting-the-github_token-permissions-for-a-specific-job) en `write` para `pull-requests`. Dependiendo de tus configuraciones de GitHub, esto puede no ser necesario. Pero para todas las organizaciones y repositorios personales [creados después del 02 de febrero de 2023](https://github.blog/changelog/2023-02-02-github-actions-updating-the-default-github_token-permissions-to-read-only/), este es el comportamiento predeterminado. Consulta [la documentación de GitHub](https://docs.github.com/es/actions/security-guides/automatic-token-authentication#modifying-the-permissions-for-the-github_token) para obtener una visión general completa.
4. [Selecciona la rama](/es/docs/explanation/branch-selection/) a utilizar:
...
6. Consulta la documentación de [ejecución de bencher](/es/docs/explanation/bencher-run/) para obtener una visión general completa de todas las formas de configurar el comentario de solicitud de extracción con las banderas `--ci-*`.
