...
It can also be nice to limit runs only after a certain label has been added (ex: `benchmark`):
...
<ul>
  ...
  <li>[Benchmark PR branch from the target branch with required reviewers](#benchmark-pr-branch-from-target-branch-with-required-reviewers)</li>
...
1. Run and track your PR benchmarks using `bencher run` directly.
...
### Benchmark PR Branch from Target Branch with Required Reviewers

<GitHubActions5 />

1. Trigger [on the `pull_request_target` event](https://docs.github.com/en/actions/using-workflows/events-that-trigger-workflows#pull_request_target).
1. If the PR is `internal` then continue on; if the PR is `external` wait for a required review.
1. Checkout the PR branch.
1. Run and track your PR benchmarks using `bencher run` directly.

This option is exactly the same as [the previous option](#benchmark-pr-branch-from-target-branch)
except it `needs` approval from a Required Reviewer before each run.
...
The `internal` environment requires no `Deployment protection rules`.
However, the `external` environment should have `Required reviewers` set to those trusted to review a PR before benchmarking.

Note that this is different than the tag option listed above for internal PRs.
The tag based solution will run _any_ future job after being tagged.
This can be useful if you only want to benchmark certain PRs.
However, tags should not be seen as a security measure as is.

The tag based approval can be limited to a single run by restricting the `on: pull_request_target` to `types: [labeled]`.
You would then have to remove and retag the PR for each subsequent approval.
At that point you're just recreating the `Deployment protection rules` above and cluttering up your PR history.
