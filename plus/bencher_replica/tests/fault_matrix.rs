...
mod cases {
    ...
    /// F10 (regression, review B1): a segment put lands server-side but
    /// reports failure, and NEW commits land before the retry. The retry must
    /// cap its scan at the failed chunk's end so it re-derives the IDENTICAL
    /// segment (an idempotent overwrite of the same key) and ships the
    /// backoff-window commits as their own segment. Without the cap the retry
    /// scans to the fresh extent and mints a longer segment with the same
    /// start; the orphaned shorter object then poisons every restore of the
    /// epoch (the offset chain forks at the duplicate start).
    #[tokio::test]
    async fn ambiguous_put_with_new_commits_reships_identical_segment() {
        let mut harness = Harness::new().await;
        harness.ready().await;
        // The next segment put reaches the backend but returns an error.
        harness.flaky().set_plan(
        ...
        assert!(
            progress.error.is_some(),
            "the lost-200 arms a backoff: {progress:?}"
        );
        // The key that persisted server-side despite the reported failure.
        let orphan_key = harness
            .flaky()
            .journal()
            .iter()
            .filter(|(op, outcome)| op.kind == OpKind::Put && *outcome == OpOutcome::InjectedAfter)
            .map(|(op, _)| op.key.clone())
            .next_back()
            .expect("the ambiguous put was journaled");

        // Fresh commits land while the backoff is armed, extending the
        // committed extent past the failed chunk's end.
        harness
            .fixture
            .txn(&["INSERT INTO t (data) VALUES ('landed-1')"])
            .expect("txn");
        harness
            .fixture
            .txn(&["INSERT INTO t (data) VALUES ('landed-2')"])
            .expect("txn");
        harness.advance(1);
        let progress = harness.engine.sync_once().await.expect("retry sync");
        assert!(progress.error.is_none(), "the retry succeeds: {progress:?}");
        assert_eq!(
            progress.shipped_segments, 2,
            "the failed chunk re-ships alone, then the backoff commits ship separately"
        );

        // The retry overwrote the exact orphaned key.
        let outcomes: Vec<OpOutcome> = harness
            .flaky()
            .journal()
            .into_iter()
            .filter(|(op, _)| op.kind == OpKind::Put && op.key == orphan_key)
            .map(|(_, outcome)| outcome)
            .collect();
        assert_eq!(
            outcomes,
            vec![OpOutcome::InjectedAfter, OpOutcome::Ok],
            "the retry re-ships the identical key, never a longer sibling"
        );

        // No two segments on the replica share (epoch directory, start).
        ...
        for key in harness
            ...
            .expect("list replica")
            .iter()
            .filter(|key| key.ends_with(".wal.zst"))
        {
            let (dir, _) = key.rsplit_once('/').expect("segment key has a directory");
            let (start, _) = segment_range(key);
            assert!(
                seen.insert((dir.to_owned(), start)),
                "duplicate-start segment on the replica: {key}"
            );
        }
        harness.assert_restore_equivalent().await;
    }
