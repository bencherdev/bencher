...
> ⚠️ It is **very**, **very** important to thoroughly review any fork PR before approving!
...
1. Create a GitHub Actions `workflow` file. (ex: `.github/workflows/pr_benchmark.yml`)
...
1. (Not Shown) Create a second GitHub Actions `workflow` file and use the initial example above to run on `push` events to the `main` branch. (ex: `.github/workflows/benchmark.yml`)
...
1. (Not Shown) Create a third GitHub Actions `workflow` file and use the initial example above to run on `push` events to the `main` branch. (ex: `.github/workflows/benchmark.yml`)
