...
- Prefer structured concurrency: every spawned task has an owner that holds its `JoinHandle` and awaits it at shutdown. Do **NOT** detach a task by dropping its handle without explicit justification - a detached task is unsupervised, its panics are silent, and shutdown cannot order itself against it
