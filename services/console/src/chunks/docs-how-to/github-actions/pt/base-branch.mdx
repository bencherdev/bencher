...
10. Configure a opção `--key` para o segredo **Repositorio** `BENCHER_API_KEY`.
   Veja [os documentos de `--key`][key option] para mais detalhes.
...
13. Configure o Limite para a Ramificação `main`, Testbed `ubuntu-latest`, e Medida `latency`:
    ...
    5. Defina a flag `--thresholds-reset` para que somente o Limite especificado esteja ativo.
       ...
       (ex: `--thresholds-reset`)
14. Defina a flag `--error-on-alert` para falhar o comando se um Alerta for gerado.
    Veja [os documentos de `--error-on-alert`][alert err] para uma visão geral completa.
    (ex: `--error-on-alert`)
