...
- `aws_s3`: Store registry artifacts in AWS S3
    ...
    - `chunk_size`: Minimum chunk size in bytes for buffering upload data before storing to S3 (default: 5 MB).
      HTTP request bodies arrive as small network frames (typically 8–64 KB). Without batching, each frame becomes a separate S3 object, creating thousands of objects per layer. This value also matches the S3 multipart upload minimum part size.
