...
8. Définissez l'option `--key` sur la variable d'environnement masquée `BENCHER_API_KEY`.
   Consultez [la documentation `--key`][key option] pour plus de détails.
   (ex : `--key "$BENCHER_API_KEY"`)
...
12. Activez le drapeau `--error-on-alert` pour échouer à la commande si une alerte est générée.
