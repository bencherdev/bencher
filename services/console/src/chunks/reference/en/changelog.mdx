...
- Deprecated: The `bencher run` options `--if-branch`, `--else-if-branch`, `--else-branch`, and `--endif-branch`
  have been deprecated in favor of using `--branch` and `--branch-start-point`
- **BREAKING CHANGE** `--if-branch` will now hard fail instead of skipping the run if not specified or if an empty string is provided
