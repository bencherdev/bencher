...
5. [`GITHUB_TOKEN`の権限][github token permissions]を`checks`と`pull-requests`に対して`write`に設定します。
...
15. アラートが生成された場合にコマンドを失敗させるために、`--error-on-alert`フラグを設定します。
...
16. Bencher Metric Format JSON (`json`)を生成する<code><a href="/ja/docs/reference/bencher-metric-format/#bencher-mock">bencher mock</a></code>を使用して、[`Bencher Metric Format JSON`（`json`）][bmf]に`--adapter`オプションを設定します。
   ...
   (例: `--adapter json`)
17. [`GITHUB_TOKEN`環境変数を使用して][github token]GitHub API認証トークンを使ってGitHubチェックおよびプルリクエストへのコメントとして結果を投稿するために、`--github-actions`オプションを設定します。
