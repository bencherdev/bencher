...
14. Defina a opção `--key` para o segredo de **Repositório** `BENCHER_API_KEY`.
    Veja [os docs de `--key`][key option] para mais detalhes.
...
19. Defina a flag `--error-on-alert` para falhar no comando se um Alerta for gerado.
   Veja [os docs de `--error-on-alert`][alert err] para uma visão completa.
   (ex: `--error-on-alert`)
20. Defina a opção `--adapter` para [Formato de Métrica Bencher JSON (`json`)][bmf] que é gerado por <code><a href="/pt/docs/reference/bencher-metric-format/#bencher-mock">bencher mock</a></code>.
   ...
   (ex: `--adapter json`)
21. Defina a opção `--github-actions` para o token de autenticação da API do GitHub para postar resultados como uma Verificação do GitHub e um comentário no Pull Request usando
...
[hash option]: /pt/docs/explanation/branches/#--hash-hash
