...
you will often want to create a new branch based on an already existing branch.
That is where the `--branch-start-point` and `branch-start-point-hash` options come in.
...
- If `branch` does exist and the start point is the same, then this option is effectively ignored.
...
- If `branch` does exist and both `branch-start-point` and the specified hash are the same,
  then this option is effectively ignored.
