...
//! reads it: every reader drives on the named `metric` rows, and those rows do not
//! exist before the migration, so no response has a pre-migration form to compare.
//! The view is therefore captured directly, and the responses that read
...
// The `metric_boundary` view is byte identical across the migration.
#[tokio::test]
async fn view_rows_are_byte_identical_across_the_migration() {
    let server = TestServer::new().await;
    let fixture = seed_legacy_project(&server, "equiv").await;

    let before = capture(&server);
    assert!(
        before.contains("40.5"),
...
/// The `metric_boundary` column list, in the order the migration's `CREATE VIEW`
/// selects it.
...
async fn seed_legacy_project(server: &TestServer, label: &str) -> Fixture {
    ...
    for iteration in 0..2 {
        ...
        for measure_id in &measure_ids {
            pairs.push((report_benchmark_id, *measure_id));
        }
    }
    ...
    for (((report_benchmark_id, measure_id), metric), metric_id) in pairs
        .into_iter()
        .zip(FIXTURE_METRICS)
...
/// Read every row of the `metric_boundary` view over a SQL connection and render it
/// to one string, which is the artifact this migration holds unchanged.
///
/// The rendering is compared, not parsed values: re-serializing would hide exactly
/// the float formatting drift this migration has to rule out.
fn capture(server: &TestServer) -> String {
    let mut conn = server.db_conn();
    diesel::sql_query(VIEW_ROWS_SQL)
