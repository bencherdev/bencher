...
and otherwise marked `unknown` (contact with the Runner was lost).
...
The limit counts from when the Job started running, or from when it was claimed if it never started, and it applies to an `unknown` Job too.
...
otherwise the Job is marked `unknown`, or `canceled` if it had exceeded the hard timeout.
The server checks an `unknown` Job again at its deadline.
On startup, the server also marks orphaned `claimed` Jobs `unknown`,
reschedules timeouts for in-flight and `unknown` Jobs,
and re-processes `completed` Jobs whose results were stored but not yet parsed;
a `completed` Job whose results were lost is marked `unknown` so the Runner can resend them.
...
A Runner's result resolves an `unknown` Job.
A `failed` or `canceled` Job never changes state again: a late result is acknowledged, so the Runner stops resending it, and discarded.
