...
/// The JSON report emitted by `vitest bench --run --outputJson <file>`.
        ...
        for file in self.files {
            for group in file.groups {
                for benchmark in group.benchmarks {
                    ...
                    // Throughput always uses `hz` (which Tinybench derives from
                    // the mean period), even under `JsonAverage::Median`, because
                    // Vitest does not expose a median-based operations per second.
...
fn combine_name(full_name: &str, name: BenchmarkName) -> BenchmarkName {
    if full_name.is_empty() {
        ...
        // If the combined name would exceed `BenchmarkName::MAX_LEN`, fall back to
        // the bare leaf `name`. Two benchmarks that share a leaf name across
        // different groups could then collide in the results map; this is only
        // reachable for pathologically long suite names.
...
pub(crate) mod test_js_vitest {
    ...
    #[test]
    fn combine_name_fallbacks() {
        use bencher_json::BenchmarkName;

        use super::combine_name;

        let add = "add".parse::<BenchmarkName>().unwrap();

        // A non-empty suite name is joined to the leaf with " > ".
        assert_eq!(
            combine_name("math.bench.ts > math", add.clone()),
            "math.bench.ts > math > add"
                .parse::<BenchmarkName>()
                .unwrap(),
        );
        ...
        assert_eq!(combine_name(&long, add.clone()), add);
    }
