...
Optional: Set the GitHub Actions `GITHUB_TOKEN` (ie `--github-token ${{ secrets.GITHUB_TOKEN }}`).
...
See [`GITHUB_TOKEN` docs](https://docs.github.com/en/actions/security-guides/automatic-token-authentication) for a full overview.
