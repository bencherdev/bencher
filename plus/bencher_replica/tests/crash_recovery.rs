...
//! Crash-recovery matrix: kill points `K_a` through `K_h`, each simulated
//! deterministically.
//!
...
pub(crate) mod harness {
    ...
    impl Harness {
        ...
        /// The resolved engine config, mutably: for knobs with no JSON
        /// field (e.g. `max_transaction_bytes`). Takes effect on the next
        /// [`Harness::rebuild_engine`].
        pub(crate) fn config_mut(&mut self) -> &mut ReplicaConfig {
            &mut self.config
        }
...
mod cases {
    ...
    /// `K_g`: graceful shutdown buries never-shipped commits. A completed
    /// checkpoint leaves the advisory meta proving "epoch shipped through
    /// checkpoint"; a storage outage then blocks every ship, so the meta is
    /// never rewritten, and `final_sync` drains incompletely. At process
    /// exit `SQLite`'s close-time checkpoint (which `wal_autocheckpoint = 0`
    /// does NOT disable) backfills the unshipped frames and resets the WAL.
    /// Without revoking the meta proof at shutdown, the next boot finds a
    /// WAL-less state the meta vouches for and continues the lineage as
    /// `AwaitingEpoch` with those commits permanently absent from the
    /// replica (later epochs replay onto a base missing them: a torn
    /// restore, not merely a stale one). The revoked proof instead forces a
    /// NEW generation whose snapshot recaptures the buried commits.
    #[tokio::test]
    async fn shutdown_incomplete_drain_then_close_checkpoint_buries_wal() {
        let mut harness = Harness::new().await;
        harness.ready().await;
        harness
            .fixture()
        ...
        assert_eq!(
            outcome,
            CheckpointOutcome::Completed,
            "the epoch is sealed by a full backfill (meta proof written)"
        );
        let old_generation = harness.engine().generation().cloned().expect("generation");

        // The outage begins; this commit restarts the fully backfilled WAL
        // and never ships. CREATE TABLE makes the eventual loss visible: no
        // later transaction rewrites the schema this touches, so nothing
        // downstream can mask a silently dropped cycle.
        harness
            .fixture()
            .txn(&[
                "CREATE TABLE buried (id INTEGER PRIMARY KEY, data TEXT)",
        ...
        // The close-time checkpoint at process exit: backfill the unshipped
        // frames into the db file and reset the WAL. A TRUNCATE checkpoint
        // leaves a zero-length WAL, which resume reads as absent, the same
        // as the deleted WAL a real connection close leaves behind.
        harness
            .fixture()
            .checkpoint(CheckpointMode::Truncate)
            .expect("close-time checkpoint");

        harness.rebuild_engine().await;
        assert_eq!(
            harness.engine().state(),
            EngineState::PendingSnapshot,
            "a WAL-less resume after an incomplete shutdown drain must force \
             a new generation, never an AwaitingEpoch continuation over \
             buried commits"
        );
        let restored = harness.assert_crash_postcondition().await;
        assert_ne!(
    ...
    /// `K_h`: graceful shutdown after a divergence buries never-shipped
    /// commits through a VACUOUSLY complete drain (the third branch of the
    /// `K_g` bug class). A completed checkpoint leaves the meta proving
    /// "epoch shipped through checkpoint"; the WAL then restarts
    /// legitimately, and the first commit of the new cycle is oversized, so
    /// the ship path diverges (position cleared, re-snapshot pending) before
    /// anything ships. `final_sync` over HEALTHY storage then drains
    /// "completely": with no bound position `ship_pass` ships nothing and
    /// reports done, the shutdown checkpoint skips (nothing provably
    /// shipped), and without the vacuous-drain revoke the meta proof
    /// survives. The close-time checkpoint backfills the new-cycle frames
    /// and resets the WAL, and the next boot would continue the lineage as
    /// `AwaitingEpoch` with those commits permanently absent from the
    /// replica. The revoked proof instead forces a NEW generation whose
    /// snapshot recaptures them.
    #[tokio::test]
    async fn shutdown_vacuous_drain_after_divergence_buries_wal() {
        let mut harness = Harness::new().await;
        // Impose a small per-transaction cap (no JSON knob: mutate the
        // resolved config and rebuild before any scenario work). The cap is
        // above a normal single-page write but well below the deliberately
        // oversized transaction later.
        harness.config_mut().max_transaction_bytes = 16 * 1024;
        harness.crash();
        harness.rebuild_engine().await;
        harness.ready().await;
        ...
        // The WAL restarts legitimately with this commit, and the commit is
        // far over the cap: the first ship pass of the new cycle diverges
        // (position cleared, re-snapshot pending) before anything ships.
        // CREATE TABLE makes the eventual loss visible: no later transaction
        // rewrites the schema this touches.
        harness
            .fixture()
            .txn(&["CREATE TABLE buried (id INTEGER PRIMARY KEY, data TEXT)"])
            .expect("txn in the new cycle");
        harness
            .fixture()
            .txn_touching_pages(16)
            .expect("oversized committed txn");
        let shipped = harness.engine_mut().ship_once().await.expect("ship");
        assert_eq!(
            shipped, 0,
            "the oversized commit diverges before anything ships"
        );

        // Graceful shutdown before the re-snapshot even starts: storage is
        // healthy, but the drain is vacuous (no bound position, nothing
        // ships) and the shutdown checkpoint skips.
        harness
            .engine_mut()
            .final_sync(Duration::from_secs(30))
            .await
        ...
        // The close-time checkpoint at process exit: backfill the unshipped
        // new-cycle frames into the db file and reset the WAL.
        harness
            .fixture()
            .checkpoint(CheckpointMode::Truncate)
            .expect("close-time checkpoint");
        ...
        assert_eq!(
            harness.engine().state(),
            EngineState::PendingSnapshot,
            "a WAL-less resume after a vacuous shutdown drain (divergence \
             pending) must force a new generation, never an AwaitingEpoch \
             continuation over buried commits"
        );
        let restored = harness.assert_crash_postcondition().await;
        assert_ne!(
            restored, old_generation,
