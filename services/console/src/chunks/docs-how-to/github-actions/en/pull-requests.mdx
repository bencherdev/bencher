...
15. Set the `--error-on-alert` flag to fail the command if an Alert is generated.
...
16. Set the `--adapter` option to [Bencher Metric Format JSON (`json`)][bmf] that is generated by <code><a href="/docs/reference/bencher-metric-format/#bencher-mock">bencher mock</a></code>.
   ...
   (ex: `--adapter json`)
17. Set the `--github-actions` option to the GitHub API authentication token to post results as a GitHub Check and a comment on the Pull Request using
...
[start point hash]: /docs/explanation/branches/#--start-point-hash-hash
[start point clone thresholds]: /docs/explanation/branches/#--start-point-clone-thresholds
[start point reset]: /docs/explanation/branches/#--start-point-reset
