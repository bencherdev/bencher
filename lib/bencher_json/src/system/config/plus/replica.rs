...
/// source. Never re-add the `litestream` section pointing at a fresh or empty
/// Litestream replica while the `replica` target holds the only copy of the
/// data: Litestream's restore precedence would restore an empty database, the
/// shadow replica would re-snapshot that empty database over the good target,
/// and retention would prune the good generations within days.
///
