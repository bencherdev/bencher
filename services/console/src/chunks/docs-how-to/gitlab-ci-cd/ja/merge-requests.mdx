...
8. `--key` オプションをマスクされた `BENCHER_API_KEY` 環境変数に設定します。
   [`--key` ドキュメント][key option] を参照してください。
   （例: `--key "$BENCHER_API_KEY"`）
...
12. アラートが発生した場合にコマンドを失敗させるために `--error-on-alert` フラグを設定します。
    [`--error-on-alert` ドキュメント][alert err] を参照して、全体像を確認してください。
    （例: `--error-on-alert`）
