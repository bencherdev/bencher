...
param (
  ...
  [string]$AppVersion = $(if ($env:BENCHER_VERSION) { $env:BENCHER_VERSION } else { '0.5.3' }),
  [Parameter(HelpMessage = "The URL of the directory where artifacts can be fetched from")]
  [string]$ArtifactDownloadUrl = "https://bencher.dev/download/$(if ($env:BENCHER_VERSION) { $env:BENCHER_VERSION } else { '0.5.3' })",
