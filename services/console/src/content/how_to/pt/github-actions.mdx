...
1. Crie um arquivo `workflow` do GitHub Actions. (ex `.github/workflows/benchmark.yml`)
...
5. O token API já deve existir. Adicione `BENCHER_API_TOKEN` como um segredo do **Repositório**. (ex: `Repo -> Configurações -> Segredos e variáveis -> Ações -> Novo segredo de repositório`) Defina a flag `--token` ou a variável de ambiente `BENCHER_API_TOKEN` para o token API. (ex: `BENCHER_API_TOKEN: ${{ secrets.BENCHER_API_TOKEN }}`)
...
7. Defina a flag `--adapter` ou a variável de ambiente `BENCHER_ADAPTER` para o nome do adaptador desejado. (ex `BENCHER_ADAPTER: json`) Se isso não for definido, então o adaptador `magic` será usado. Consulte [adaptadores de sistema de referência](/docs/pt/explanation/adapters) para uma visão geral completa.
...
10. [Rastreie seus benchmarks](/docs/pt/how-to/track-benchmarks) com o subcomando `bencher run` do CLI:
    ...
    2.  Configure o comando para falhar se um Alerta for gerado. (ex: `--err`) Para que um Alerta seja gerado, um [Limiar](/docs/pt/explanation/thresholds) já deve existir.
...
1. Crie um arquivo `workflow` do GitHub Actions. (ex: `.github/workflows/benchmark.yml`)
1. Execute em eventos `push` para a branch `main` e em eventos `pull_request`. É importante limitar a execução em `push` **apenas** para as branches selecionadas (ex: `main`) para prevenir que os `pushes` para as branches do PR sejam executados duas vezes!
2. Em vez de sempre usar a branch `main`, use as [variáveis de ambiente padrão do GitHub Action](https://docs.github.com/pt/actions/learn-github-actions/variables#default-environment-variables) para:
        1. Usar os dados da branch atual, se já existirem. (ex: `--if-branch '${{ github.head_ref }}'`)
        2. Criar um clone dos dados da branch alvo do PR e limiares, se já existirem. (ex: `--else-if-branch '${{ github.base_ref }}'`)
        3. Caso contrário, crie um clone dos dados da branch `main` e limiares. (ex: `--else-if-branch main`)
        4. Existem várias opções para definir a branch do projeto. Consulte [seleção de branch](/docs/pt/explanation/branch-selection) para uma visão geral completa.
3. Defina o token de autenticação da API do GitHub. (ex: `--github-actions "${{ secrets.GITHUB_TOKEN }}"`) Quando esta opção é definida como parte de uma pull request, os resultados serão adicionados à solicitação de pull como um comentário. Isso usa [a variável de ambiente `GITHUB_TOKEN` do GitHub Actions](https://docs.github.com/pt/actions/security-guides/automatic-token-authentication).
4. Consulte a documentação do [bencher run](/docs/pt/explanation/bencher-run) para uma visão geral completa de todas as maneiras de configurar o comentário da solicitação de extração com as flags `--ci-*`.
...
1. Crie um arquivo `workflow` do GitHub Actions. (ex: `.github/workflows/pr_benchmark.yml`)
...
1. (Não demonstrado) Crie um segundo arquivo `workflow` das GitHub Actions e use o exemplo inicial acima para executar em eventos `push` para a branch `main`. (ex: `.github/workflows/benchmark.yml`)
...
1. Crie um segundo arquivo de workflow, `Track Benchmarks`.
...
1. (Não demonstrado) Crie um terceiro arquivo `workflow` das GitHub Actions e use o exemplo inicial acima para executar em eventos `push` para a branch `main`. (ex: `.github/workflows/benchmark.yml`)
...
<h2><a href="/docs/pt/explanation/benchmarking">Continuar: Análise de Desempenho ➡</a></h2>
