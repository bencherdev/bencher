...
그러나 경우에 따라, 플랫폼의 기본 인증서 스토어를 사용하고 싶을 수 있습니다. 특히 시스템의 인증서 스토어에 포함된 기업 신뢰 루트를 의존하는 경우, 필수 프록시 또는 자체 서명된 Bencher 셀프 호스팅 연결을 위해 이와 같은 설정이 유용할 수 있습니다. `bencher`가 시스템의 신뢰 스토어를 사용하도록 하려면, [`--native-tls` flag][native tls]를 사용하여 `bencher`를 실행하십시오.
...
마지막으로, 자가 서명된 인증서를 신뢰하거나 인증서 검증을 비활성화하려는 경우 설정이 있는 경우, [`--insecure-host` flag][insecure host]를 사용하여 특정 호스트로의 불안전한 연결을 허용하도록 `bencher`에 지시할 수 있습니다. 이 플래그는 HTTPS 연결에만 적용되며, HTTP 연결은 본질적으로 불안전합니다.
