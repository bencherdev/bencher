...
- Wenn `branch` nicht existiert und der `start-point` am angegebenen Hash nicht existiert,
  wird der `branch` aus der neuesten `start-point`-Version erstellt.
  Wenn `start-point` nicht existiert, wird der `branch` ohne historische Daten erstellt.
- Wenn `branch` existiert und der `start-point` am angegebenen Hash nicht existiert,
  wird ein neuer `branch`-[Head][head] aus der neuesten `start-point`-Version erstellt.
  Wenn `start-point` nicht existiert, wird diese Option ignoriert.
