...
/// Streaming upload to S3 via multipart. Buffers writes internally and
/// uploads every non-last part at exactly the 5 MiB minimum part size
/// (Cloudflare R2 requires all non-last parts to be the same size, so part
/// boundaries must not follow chunk arrival). Dropping without
/// [`Self::finish`] leaves only an uncompleted multipart upload: the object
/// never becomes visible under its key.
...
impl S3Multipart {
    pub(crate) async fn write_part(&mut self, bytes: Bytes) -> Result<(), StorageError> {
        ...
        for body in drain_full_parts(&mut self.buffer) {
            self.upload_part_body(body).await?;
        }
        Ok(())
    }
    ...
    async fn upload_part_body(&mut self, body: Vec<u8>) -> Result<(), StorageError> {
...
/// Split every complete `MIN_PART_BYTES` slice off the front of the buffer,
/// leaving the remainder (possibly empty) in place. Every non-last part must
/// be exactly `MIN_PART_BYTES`: Cloudflare R2 rejects
/// `CompleteMultipartUpload` when non-last parts differ in size, so part
/// boundaries are fixed rather than following chunk arrival.
fn drain_full_parts(buffer: &mut Vec<u8>) -> Vec<Vec<u8>> {
    let mut parts = Vec::new();
    while buffer.len() >= MIN_PART_BYTES {
        let remainder = buffer.split_off(MIN_PART_BYTES);
...
mod tests {
    ...
    #[test]
    fn multipart_non_last_parts_are_uniform() {
        // Feed chunks whose size is deliberately misaligned with the part
        // size, as WAL-segment and snapshot chunk boundaries are. R2 rejects
        // CompleteMultipartUpload when non-last parts differ in size, so
        // every drained part must be exactly MIN_PART_BYTES regardless of
        // how the chunks land.
        let chunk_len = 3 * 1024 * 1024 + 17;
        let mut buffer = Vec::new();
        let mut parts = Vec::new();
        let mut written = Vec::new();
        for round in 0u8..7 {
            // Each round has a distinct fill byte so reordering or loss
            // across part boundaries is detectable, not just length drift.
            let chunk = vec![round; chunk_len];
            written.extend_from_slice(&chunk);
            buffer.extend_from_slice(&chunk);
            parts.extend(drain_full_parts(&mut buffer));
        }
        assert!(
            parts.iter().all(|part| part.len() == MIN_PART_BYTES),
            "every non-last part must be exactly MIN_PART_BYTES, got sizes: {:?}",
            parts.iter().map(Vec::len).collect::<Vec<_>>()
        );
        assert!(
            buffer.len() < MIN_PART_BYTES,
            "the carried remainder must be smaller than a full part"
        );
        let mut reassembled = parts.concat();
        reassembled.extend_from_slice(&buffer);
        assert_eq!(
            reassembled, written,
            "parts plus remainder must reproduce the input byte-for-byte"
        );
    }

    #[test]
    fn drain_full_parts_leaves_short_buffers_untouched() {
        let mut buffer = vec![7u8; MIN_PART_BYTES - 1];
        assert!(
            drain_full_parts(&mut buffer).is_empty(),
            "a sub-minimum buffer must not produce a part"
        );
        assert_eq!(
            buffer.len(),
            MIN_PART_BYTES - 1,
            "the buffer must be left intact"
        );
    }
