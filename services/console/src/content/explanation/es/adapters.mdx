...
<a href="https://bencher.dev/perf/bencher?key=true&metric_kind=latency&tab=benchmarks&testbeds=0d991aac-b241-493a-8b0f-8d41419455d2&branches=619d15ed-0fbd-4ccb-86cb-fddf3124da29&benchmarks=3525f177-fc8f-4a92-bd2f-dda7c4e15699%2C1db23e93-f909-40aa-bf42-838cc7ae05f5&start_time=1674950400000"><img style="border: 0.2em solid #ed6704;" src="https://api.bencher.dev/v0/projects/bencher/perf/img?branches=619d15ed-0fbd-4ccb-86cb-fddf3124da29&testbeds=0d991aac-b241-493a-8b0f-8d41419455d2&benchmarks=3525f177-fc8f-4a92-bd2f-dda7c4e15699%2C1db23e93-f909-40aa-bf42-838cc7ae05f5&metric_kind=latency&start_time=1674950400000&title=Benchmark+Adapter+Comparison" title="Comparativa de Adaptadores de Referencia" alt="Comparativa de Adaptadores de Referencia para Bencher - Bencher" /></a>
</div>
...
El objeto `benchmark_name` contiene los alias de Metric Kind o UUIDs como claves.
En este ejemplo, `latency` es el slug para Latency Metric Kind.
Cada proyecto por defecto tiene una Latencia (es decir `latency`) y Throughput (es decir `throughput`) Metric Kind,
que se miden en `nanosegundos (ns)` y `operaciones / segundo (ops/s)` respectivamente.
El objeto Metric Kind contiene una Métrica con hasta tres medidas: `value`, `lower_value` y `upper_value`.
...
El adaptador C++ Google (`cpp_google`) espera la salida de [Google Benchmark](https://github.com/google/benchmark) en [formato JSON (es decir `--benchmark_format=json`)](https://github.com/google/benchmark/blob/main/docs/es/user_guide.md#output-formats).
Se recoge la Latency Metric Kind (`nanosecond (ns)`).
...
El adaptador JavaScript Time (`js_time`) espera la salida de [console.time](https://developer.mozilla.org/en-US/docs/es/Web/API/console/time)/[console.timeEnd](https://developer.mozilla.org/en-US/docs/es/Web/API/console/timeEnd).
La Latency Metric Kind (`nanoseconds (ns)`) se recoge.
...
La Latency Metric Kind (`nanosecond (ns)`) se recoge.
...
La Metric Kind `latency` (`nanoseconds (ns)`) se recoge.
...
Se recogen las Latency Metric Kinds `instructions`, `l1_access`, `l2_access`, `ram_access`, y `estimated_cycles`.
