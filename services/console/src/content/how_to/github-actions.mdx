...
1. Create a GitHub Actions `workflow` (ex: `.github/workflows/benchmark.yml`) and make sure that it at least runs on both `push` and `pull_request` events. (ex: `on: [push, pull_request]`). See the [GitHub Actions `on` documentation](https://docs.github.com/en/actions/using-workflows/workflow-syntax-for-github-actions#on) for a full overview.
