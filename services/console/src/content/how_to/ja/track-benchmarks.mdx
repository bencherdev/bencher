...
より詳しい説明については、[ベンチマーキング概要](/docs/ja/explanation/benchmarking)をご覧ください。
以下は、`feature-branch`という適切な名前のフィーチャーブランチでベンチマークを追跡するための<code><a href="/docs/ja/explanation/bencher-run">bencher run</a></code> CLIサブコマンドの例です：
...
1. オプション：`--adapter`フラグまたは`BENCHER_ADAPTER`環境変数を希望のアダプタ名に設定します。これが設定されていない場合、`magic`アダプタが使用されます。全体の概要については[ベンチマークハーネスアダプタ](/docs/ja/explanation/adapters)を参照してください。(例：`--adapter json`)
1. プロジェクトブランチの設定にはいくつかのオプションがあります。完全な概要については[ブランチの選択](/docs/ja/explanation/branch-selection)を参照してください。
...
1. アラートが生成された場合にコマンドを失敗させるように設定します。アラートを生成するためには、[しきい値](/docs/ja/explanation/thresholds)が既に存在している必要があります。(例：`--err`)
...
以下は、`feature-branch`という適切な名前のフィーチャーブランチで相対ベンチマーキングを実行するための<code><a href="/docs/ja/explanation/bencher-run">bencher run</a></code> CLIサブコマンドの例です：
...
1. フィーチャーブランチをチェックアウトします。(例：`feature-branch`)
1. フィーチャーブランチのための[しきい値](/docs/ja/explanation/thresholds)を作成します：
    ...
    1. パフォーマンスが後退することを示すため、Latencyが大きいほど右側の境界を95.0%に設定します。(例：`--right-side 0.95`)
1. フィーチャーブランチに対して `bencher run` を実行します：
    ...
    1. [しきい値](/docs/ja/explanation/thresholds)によりアラートが生成された場合にコマンドが失敗するように設定します。(例：`--err`)
...
<h2><a href="/docs/ja/how-to/github-actions">BencherをGitHub Actionsに追加 ➡</a></h2>
<h2><a href="/docs/ja/how-to/gitlab-ci-cd">BencherをGitLab CI/CDに追加 ➡</a></h2>
