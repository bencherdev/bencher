...
5. Establece [los permisos para el `GITHUB_TOKEN`][github token permissions]
   a `write` para `checks` y `pull-requests`.
...
15. Establece la bandera `--error-on-alert` para fallar el comando si se genera una Alerta.
...
16. Establece la opción `--adapter` al [Formato de Métrica Bencher JSON (`json`)][bmf] que es generado por <code><a href="/es/docs/reference/bencher-metric-format/#bencher-mock">bencher mock</a></code>.
   ...
   (ej: `--adapter json`)
17. Establece la opción `--github-actions` al token de autenticación de la API de GitHub para publicar resultados como una Verificación de GitHub y un comentario en la Pull Request usando
