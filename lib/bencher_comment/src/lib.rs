...
pub struct ReportComment {
    ...
    variant_benchmarks: HashSet<BenchmarkUuid>,
...
impl ReportComment {
    pub fn new(
        ...
        Self {
            ...
            variant_benchmarks: variant_benchmarks(&json_report),
            json_report,
            sub_adapter,
            source,
        }
    ...
    /// The name a row gives its benchmark, and the variant that row carries.
    ///
    /// A benchmark whose every row in this comment carries the empty variant keeps
    /// the bare benchmark name it has always had. One non-empty variant among its
    /// rows names them all, the empty variant among them included, which reads `{}`.
    /// The parameters are spelled in their canonical form and follow the benchmark
    /// name, the way the perf image spells them.
    fn benchmark_label(&self, benchmark: &JsonBenchmark, parameters: &ParameterSet) -> String {
        if self.variant_benchmarks.contains(&benchmark.uuid) {
            format!(
                "{name} {parameters}",
                name = benchmark.name,
    ...
    /// The name a row gives a checked measure: the measure, and the metric the
    /// threshold checks when that is not the conventional `value`.
    ///
    /// A threshold that names no metric checks `value`, so it reads as the measure
            ...
            for result in iteration {
                for report_measure in &result.measures {
                    text.push_str(&format!(
                        ...
                        benchmark =
                            self.benchmark_label(&result.benchmark, &result.variant.parameters),
    ...
    fn human_alerts_list(&self, text: &mut String) {
        ...
        for alert in self.alerts() {
            text.push_str(&format!(
                ...
                benchmark_name = self.benchmark_label(&alert.benchmark, &alert.variant.parameters),
                measure_name =
    ...
    fn html_alerts_table_body(&self, html: &mut String) {
        ...
        for alert in self.alerts() {
            ...
            html.push_str(&format!(
                ...
                benchmark = self.benchmark_label(&alert.benchmark, &alert.variant.parameters),
            ));
            html.push_str(&format!(
    ...
    fn html_iteration_table_body(
        ...
        for result in iteration {
            ...
            html.push_str(&format!(
                ...
                name = self.benchmark_label(&result.benchmark, &result.variant.parameters),
            ));
            for (measure, measure_columns) in columns {
    ...
    fn html_iteration_table_measure_cells(
        ...
        for name in &measure_columns.names {
            ...
            if let Some(named) = named {
                // A metric has one column, so what a threshold computed
    ...
    ///
    /// Four dimensions are matched: the benchmark, the variant, the measure, and
    /// the name. Two variants of one benchmark are two rows and two names of one
    ...
    pub fn find_alert(
        ...
        self.alerts().iter().find(|alert| {
            alert.benchmark.slug == result.benchmark.slug
                && alert.variant.uuid == result.variant.uuid
...
/// The threshold this measure's cell links to, if anything checked it.
///
/// The deprecated singular threshold is the bare one, which checks the conventional
/// `value` name of every variant. Every threshold a BMF v0 payload can create is
/// bare, so a v0 measure that anything checked has it and this never looks any
/// further: the v0 cell is the cell it always was. A measure checked only by a
/// threshold that names a metric or filters variants has to be read off the rows
...
/// Whether anything checked this measure of this variant.
///
...
/// Every benchmark whose rows in this comment name the variant they carry.
///
/// A benchmark is in here when any row of it, result or alert, carries non-empty
/// parameters. Results and alerts are counted together so the two tables of one
/// comment never disagree about how a benchmark is named.
fn variant_benchmarks(json_report: &JsonReport) -> HashSet<BenchmarkUuid> {
    let mut benchmarks = HashSet::new();
    for iteration in json_report.results.as_deref().unwrap_or_default() {
        for result in iteration {
            if !result.variant.parameters.is_empty() {
                benchmarks.insert(result.benchmark.uuid);
            }
        }
    }
    for alert in json_report.alerts.as_deref().unwrap_or_default() {
        if !alert.variant.parameters.is_empty() {
            benchmarks.insert(alert.benchmark.uuid);
        }
    }
    benchmarks
}
...
fn measure_columns(
    ...
    for result in iteration {
        for report_measure in &result.measures {
            ...
            // `--ci-only-thresholds` only a checked name does, the same way only a
...
mod tests {
    ...
    /// estimate and one that named only a percentile, with nothing checking either.
    fn value_less_results() -> JsonReportResults {
        value_less_results_checked(&serde_json::json!([]))
    }
    ...
    fn named_check() -> serde_json::Value {
    ...
    fn value_less_results_checked(boundaries: &serde_json::Value) -> JsonReportResults {
        let date = serde_json::to_value(DateTime::TEST).unwrap();
    ...
    /// One iteration of one benchmark on `sets`, one variant per set, each with
    /// one measure that named a point estimate.
    ///
    /// Built from JSON rather than constructors because the parameters on the wire
    /// are what is under test.
    fn variant_results(sets: &[serde_json::Value]) -> JsonReportResults {
        let date = serde_json::to_value(DateTime::TEST).unwrap();
        let results = sets
            .iter()
            .enumerate()
    ...
    /// One alert on the `p99` row of one variant of `bench`.
    ...
    fn report_table_value_less_measure() {
        ...
        assert!(
            ...
            "the metric has a column of its own: {html}"
        );
        assert!(
            html.contains("<td>2.00 ns</td>"),
            "the metric is drawn: {html}"
        );
    ...
    // A measure checked only by a threshold that names a metric has no bare threshold,
    // so the deprecated singular field is absent. The cell links to the threshold
    // that did check it and never shouts NO THRESHOLD, which is what keeps the cell
    // and the report level warning telling the same story about one measure.
    #[test]
    fn report_table_named_threshold_checks_the_cell() {
        let mut report = json_report(Visibility::Public);
        report.results = Some(value_less_results_checked(&named_check()));

        let comment = report_comment_for(report);
        let html = comment.html(false, None);

        // The measure the named threshold checked: linked, not warned about.
        assert!(
            html.contains(
                "/thresholds/eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee?model=ffffffff-ffff-ffff-ffff-ffffffffffff\">view threshold</a>"
            ),
            "the cell links the threshold that checked the metric: {html}"
        );
        ...
        assert!(
            ...
            "the warning does not name a measure something checked: {html}"
        );
        // The measure nothing checked is still warned about, in the cell and above it.
        assert_eq!(
            html.matches("⚠️ NO THRESHOLD").count(),
            1,
            "only the unchecked measure's cell says so: {html}"
        );
        assert!(
            html.contains("/measures/latency\">Latency (nanoseconds (ns))</a>"),
            "the warning still names the unchecked measure: {html}"
        );
        // One checked measure is enough for `--ci-only-thresholds` to post.
        assert!(comment.has_threshold(), "unexpected report: {html}");
    }

    // Nothing checked either measure, so every reader agrees the other way too.
    #[test]
    fn report_table_unchecked_measures_have_no_threshold() {
        let mut report = json_report(Visibility::Public);
        report.results = Some(value_less_results());
    ...
    fn report_table_v0_has_no_named_columns() {
        let mut report = json_report(Visibility::Public);
        report.results = Some(variant_results(&[serde_json::json!({})]));
    ...
    // A benchmark whose every row in the comment carries the empty variant keeps
    // the bare benchmark name it has always had, in the table and in the human text
    // alike.
    #[test]
    fn report_labels_stay_bare_without_variants() {
        let mut report = json_report(Visibility::Public);
        report.results = Some(variant_results(&[serde_json::json!({})]));
        let comment = report_comment_for(report);
        ...
        assert!(
            !html.contains("bench {}"),
            "a benchmark with no variants is named the way it always was: {html}"
        );
        assert!(
    ...
    // Two variants of one benchmark are two rows, so each row names the set it
    // carries, and the empty set among them reads `{}`.
    #[test]
    fn report_labels_name_each_variant() {
        let mut report = json_report(Visibility::Public);
        report.results = Some(variant_results(&[
            serde_json::json!({}),
    ...
    // The alert table reads the flat value, names the variant the alert fired on,
    // and names the metric its threshold checks when that is not `value`.
    #[test]
    fn report_alert_names_the_variant_and_the_metric() {
        let mut report = json_report(Visibility::Public);
        report.results = Some(variant_results(&[serde_json::json!({ "size_mb": 16 })]));
        report.alerts = Some(named_alert(&serde_json::json!({ "size_mb": 16 })));

        let html = report_comment_for(report).html(false, None);
        assert!(
            html.contains(r#"/benchmarks/bench">bench {"size_mb":16}</a>"#),
            "the alert names the variant it fired on: {html}"
        );
        assert!(
            html.contains(">Throughput (p99)<br />nanoseconds (ns)</a>"),
            "the alert names the metric its threshold checks: {html}"
        );
    ...
    // A threshold that names no metric checks the conventional `value`, so the alert
