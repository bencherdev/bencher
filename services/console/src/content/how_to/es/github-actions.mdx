...
1. Crea un archivo `workflow` de GitHub Actions. (ej: `.github/workflows/benchmark.yml`)
...
5. La ficha API ya debe existir. Añade `BENCHER_API_TOKEN` como un secreto del **repositorio**. (ej: `Repo -> Settings -> Secrets and variables -> Actions -> New repository secret`) Establece la bandera `--token` o la variable de entorno `BENCHER_API_TOKEN` a la ficha API. (ej: `BENCHER_API_TOKEN: ${{ secrets.BENCHER_API_TOKEN }}`)
...
1. Crea un archivo `workflow` de GitHub Actions. (ej: `.github/workflows/benchmark.yml`)
1. Ejecuta en eventos `push` a la rama `main` y en eventos `pull_request`. Es importante limitar la ejecución en `push` **solo** a las ramas seleccionadas (ej: `main`) para evitar que las push a las ramas de PR se ejecuten dos veces!
2. En lugar de usar siempre la rama `main`, usa las [variables de entorno predeterminadas de GitHub Action](https://docs.github.com/es/actions/learn-github-actions/variables#default-environment-variables) para:
        1. Usa los datos de la rama actual si ya existen. (ej: `--if-branch '${{ github.head_ref }}'`)
        2. Crea un clon de los datos de la rama objetivo de PR y umbrales si ya existen. (ej: `--else-if-branch '${{ github.base_ref }}'`)
        3. De lo contrario, crea un clon de los datos de la rama `main` y umbrales. (ej: `--else-if-branch main`)
        4. Hay varias opciones para configurar la rama del proyecto. Consulta la [selección de rama](/docs/es/explanation/branch-selection) para una visión completa.
3. Establece el token de autenticación de la API de GitHub. (ej: `--github-actions "${{ secrets.GITHUB_TOKEN }}"`) Cuando esta opción se configura como parte de un pull request, entonces los resultados se añadirán al pull request como un comentario. Esto usa [la variable de entorno `GITHUB_TOKEN` de GitHub Actions](https://docs.github.com/es/actions/security-guides/automatic-token-authentication).
4. Consulta la documentación de [bencher run](/docs/es/explanation/bencher-run) para una visión completa de todas las formas de configurar el comentario de la pull request con las banderas `--ci-*`.
...
1. Crea un archivo `workflow` de GitHub Actions. (ej: `.github/workflows/pr_benchmark.yml`)
...
1. (No se muestra) Crea un segundo archivo `workflow` de GitHub Actions y usa el ejemplo inicial de arriba para ejecutar en eventos `push` a la rama `main`. (ej: `.github/workflows/benchmark.yml`)
...
1. (No se muestra) Crea un tercer archivo de flujo de trabajo de GitHub Actions y usa el ejemplo inicial de arriba para ejecutar en eventos de `push` a la rama `main`. (ej: `.github/workflows/benchmark.yml`)
...
<h2><a href="/docs/es/explanation/benchmarking">Continúa: Visión General del Benchmarking ➡</a></h2>
