...
There are three replication `scheme`s:
...
- `checkpoint_interval`: (Optional) How often Litestream performs a non-blocking PASSIVE checkpoint regardless of page count. Litestream skips the checkpoint if readers or writers are active. Defaults to `1m`.
- `min_checkpoint_page_count`: (Optional) Minimum number of WAL pages before a non-blocking PASSIVE checkpoint is triggered (~4KB per page). Litestream skips the checkpoint if readers or writers are active. Defaults to `1000` (~4MB).
