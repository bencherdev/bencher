...
Auf dem `stable`-Update-Kanal wird ein `update` gesendet, wenn die Runner-Version von der Server-Version abweicht.
Auf dem `canary`-Update-Kanal ist `version` gleich `canary`,
und ein `update` wird gesendet, wenn die vom Runner selbst gemeldete Prüfsumme des Binaries
vom veröffentlichten rollierenden Canary-Build abweicht.
