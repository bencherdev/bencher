...
- `branch`가 존재하지 않고 지정된 해시의 `start-point`가 존재하지 않으면,
  `branch`는 `start-point`의 최신 버전에서 생성됩니다.
  `start-point`가 존재하지 않으면, `branch`는 아무런 기록 데이터 없이 생성됩니다.
- `branch`가 존재하고 지정된 해시에 `start-point`가 존재하지 않으면,
  `start-point`의 최신 버전에서 새로운 `branch` [헤드][head]가 생성됩니다.
  `start-point`가 존재하지 않으면, 이 옵션은 무시됩니다.
