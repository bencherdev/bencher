...
### 콘솔 UI
`bencher-ui` Docker 이미지를 실행할 때 `GITHUB_CLIENT_ID` 환경 변수를 설정합니다. (예: `GITHUB_CLIENT_ID=Iv1.12864abcd1232048`) 만약 클라이언트 ID가 누락되면, UI에 GitHub 버튼이 표시되지 않습니다.

### API 서버
`client_id`와 `client_secret`를 [서버 구성의 `plus.github` 섹션](/docs/ko/reference/server-config/#plusgithub)에 추가합니다. 만약 GitHub 섹션이 누락되면, 서버는 GitHub 인증 요청을 실패시킵니다.
...
<h2><a href="/docs/ko/how-to/github-actions/">Bencher를 GitHub Actions에 추가하기 ➡</a></h2>