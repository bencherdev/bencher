...
A [project API key](#--key-key) cannot be used to create a new Project or claim an existing Project, so it requires `--project`/`BENCHER_PROJECT` to be set to an existing Project.
If the value specified is a slug and the Project does not already exist, it will be created for you when a user API key ([the `--key` option](#--key-key)), [the deprecated `--token` option](#--token-token), or no credentials at all are used.
...
If the user is authenticated with a user API key ([the `--key` option](#--key-key)) or [the deprecated `--token` option](#--token-token), then the Project is added under that user's personal Organization.
