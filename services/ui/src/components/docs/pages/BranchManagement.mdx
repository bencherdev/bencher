...
It is also possible to use mutliple `--else-if-branch` flags. For example if `EMPTY_ENV_VAR` is an empty environment variable, `--if-branch feature_branch --else-if-branch $EMPTY_ENV_VAR --else-if-branch main` would function the same as the previous example, copying all of the data from `main` and proceeding.
