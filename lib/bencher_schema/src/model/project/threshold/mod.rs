...
impl InsertThreshold {
    ...
    /// testbed.
    ///
    /// The report's BMF version, declared or taken from the project, says which
    /// shape the thresholds are written in, and it is the version rather than the
    /// shape that decides: a payload in the other version's shape is refused.
    #[expect(
    ...
    pub async fn from_report_json(
        ...
        // Ingest already refused a mismatched shape before it created anything.
        // This keeps the check with the code that relies on it,
        ...
        // bare one: the `value` name of every variant. A threshold that checks a
        // name or only some variants is not expressible in that shape, so a legacy
        // run cannot strip one it cannot even name. A version 1 entry names
        // everything a threshold checks, so the list reaches every threshold on the
...
///
/// Called before anything at all is created for the report: a payload that is going
/// to be turned away should not leave a branch, a testbed, or a measure behind on
/// its way out.
...
/// Refuse a thresholds shape that is not the one the payload's BMF version,
/// declared or taken from the project, spells.
///
/// The version is what discriminates, not the shape that arrived: a version 0 map
/// read at version 1 is as wrong as the reverse, and both refusals name the version
/// the report is read as and the shape it expects.
fn check_models_shape(
    bmf_version: BmfVersion,
    models: Option<&JsonReportThresholdModels>,
) -> Result<(), HttpError> {
    let expected = if bmf_version == BmfVersion::V0 {
...
mod shape_tests {
    ...
    /// refusals name the version the report is read as and the shape it expects.
    #[test]
