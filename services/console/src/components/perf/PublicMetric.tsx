...
import type { JsonMetric, JsonOneMetric } from "../../types/bencher";
...
export interface Props {
	apiUrl: string;
	params: Params;
	data: undefined | JsonOneMetric;
}

const PublicMetric = (props: Props) => {
	...
	const title = createMemo(
		() =>
			`${props.data?.branch?.name} | ${props.data?.testbed?.name} | ${props.data?.benchmark?.name} | ${props.data?.measure?.name}`,
	);
	...
	const getData = async (_bencher_valid: InitOutput) => {
		...
		const path = `/v0/projects/${props.params?.project}/metrics/${props.params?.metric}`;
		return await httpGet(props.apiUrl, path, null)
			.then((resp) => {
				return resp?.data as JsonMetric;
			})
			.catch((error) => {
				console.error(error);
	...
	const [metricData, { refetch }] = createResource(bencher_valid, getData);
