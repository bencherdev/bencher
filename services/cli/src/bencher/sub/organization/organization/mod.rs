use crate::{CliError, bencher::sub::SubCmd, parser::organization::CliOrganization};
...
impl TryFrom<CliOrganization> for Organization {
    ...
    fn try_from(branch: CliOrganization) -> Result<Self, Self::Error> {
        Ok(match branch {
            ...
            CliOrganization::Claim(claim) => Self::Claim(claim.try_into()?),
...
impl SubCmd for Organization {
    async fn exec(&self) -> Result<(), CliError> {
        match self {
            ...
            Self::Claim(claim) => claim.exec().await,
