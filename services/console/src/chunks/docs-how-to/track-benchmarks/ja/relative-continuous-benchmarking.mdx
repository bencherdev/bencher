...
[クイックスタート][quick start]および[Docker 自己ホスト][docker self-hosted]チュートリアルから引き継いで、`Save Walter White`プロジェクトに相対[連続ベンチマーク][continuous benchmarking]を追加してみましょう。
...
CIでこのコマンドを初めて実行すると、存在しないため`feature-branch`ブランチが作成されます。
新しい`feature-branch`には、開始点、既存のデータ、またはスレッショルドがありません。
その後の実行では、古い`feature-branch`[ヘッド][head]が置き換えられ、新しい`feature-branch`[ヘッド][head]が開始点、既存のデータ、またはスレッショルドなしで作成されます。
