...
15. Définissez le drapeau `--error-on-alert` pour échouer la commande si une Alerte est générée.
...
16. Définissez l'option `--adapter` sur le Format de Métrique Bencher JSON (`json`) généré par <code><a href="/fr/docs/reference/bencher-metric-format/#bencher-mock">bencher mock</a></code>.
   ...
   (ex : `--adapter json`)
17. Définissez l'option `--github-actions` sur le token d'authentification de l'API GitHub pour poster les résultats en tant que vérification GitHub et commentaire sur la Pull Request en utilisant
