...
For security reasons, secrets such as your `BENCHER_API_KEY` and the `GITHUB_TOKEN` are not available in GitHub Actions for fork PRs.
...
No secrets such as your `BENCHER_API_KEY` and the `GITHUB_TOKEN` are available there.
...
where secrets such as your `BENCHER_API_KEY` and the `GITHUB_TOKEN` are available.
