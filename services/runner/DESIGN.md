...
3. **Idempotency:** All handlers already handle duplicate terminal messages safely. Sending Completed for an already-Completed or Processed job is a no-op. Sending Failed for an already-Failed job is a no-op. This means retries are always safe. Additionally, a heartbeat-timeout-induced Failed status can be overridden by actual results from the runner: Completed for a Failed job transitions to Completed and processes results normally.
...
| failed    | completed | Runner resends `completed` event (overrides heartbeat-timeout-induced failure) |
...
**Terminal states:** processed, failed, canceled (no transitions out, except failed→completed for result recovery). `completed` is quasi-terminal: transitions to `processed` once results are parsed.
...
```
1. Idle: Connect to WebSocket /v0/runners/{runner}/channel, send Ready
                │
                ▼ (Server sends Job with JsonClaimedJob containing config, OCI token, and full spec)
