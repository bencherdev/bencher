...
[Руководствах Быстрого Старта][quickstart] и [Docker для Самостоятельного Развертывания][docker self-hosted],
давайте добавим Относительное [Непрерывное Бенчмаркинг][continuous benchmarking] в наш проект `claimed`.

> 🐰 Убедитесь, что вы
> [создали токен API и установите его в качестве переменной окружения `BENCHER_API_KEY`][create an api token]
> перед продолжением!
