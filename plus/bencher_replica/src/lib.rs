...
//! - **Transactions larger than the 4 GiB shippable bound degrade
//!   replication.** A single committed transaction whose WAL footprint
//!   exceeds `max_transaction_bytes` cannot ship as a segment; the engine
//!   captures it with a whole-database re-snapshot and drains the WAL, and
//!   under sustained writes the drain only converges in a quiet window (the
//!   `replica.oversized.stalled` counter fires while it is stalled and the
//!   re-snapshot retries on the oversized retrigger cadence). Until then WAL
//!   segments do not ship and the local WAL grows. In particular, avoid
//!   `VACUUM` on a replicated database whose file size is near or beyond the
//!   bound: it rewrites the whole database in one transaction.
