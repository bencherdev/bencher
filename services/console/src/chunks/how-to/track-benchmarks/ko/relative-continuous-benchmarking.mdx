...
[빠른 시작][quick start] 및 [자체 호스팅 Docker][docker self-hosted] 튜토리얼에서 이어서, `Save Walter White` 프로젝트에 상대적 [지속적 벤치마킹][continuous benchmarking]을 추가하겠습니다.
...
이 명령이 CI에서 처음 실행될 때, 아직 존재하지 않으므로 `feature-branch` Branch가 생성됩니다. 새로운 `feature-branch`에는 시작 지점, 기존 데이터 또는 임계값이 없습니다. 이후의 실행에서는 이전 `feature-branch` [머리][head]가 대체되고 시작 지점, 기존 데이터 또는 임계값 없이 새로운 `feature-branch` [머리][head]가 생성됩니다.
