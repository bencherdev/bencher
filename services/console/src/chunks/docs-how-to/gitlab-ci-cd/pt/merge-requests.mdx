...
8. Defina a opção `--key` para a variável de ambiente mascarada `BENCHER_API_KEY`.
   Veja [a documentação de `--key`][key option] para mais detalhes.
...
12. Defina o flag `--error-on-alert` para falhar o comando se um Alerta for gerado.
   Veja [a documentação de `--error-on-alert`][alert err] para uma visão completa.
   (ex: `--error-on-alert`)
