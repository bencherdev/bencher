...
6. `GITHUB_TOKEN`에 대한 [권한을][github token permissions] `checks` 및 `pull-requests`에 대해 `write`로 설정합니다.
...
14. `--key` 옵션을 `BENCHER_API_KEY` **Repository** 시크릿으로 설정합니다.
    더 자세한 내용은 [`--key` 문서][key option]를 참조하세요.
...
19. 경고가 생성되면 명령을 실패 처리하는 `--error-on-alert` 플래그를 설정합니다.
...
20. <code><a href="/ko/docs/reference/bencher-metric-format/#bencher-mock">bencher mock</a></code>에 의해 생성된 [Bencher Metric Format JSON (`json`)][bmf]으로 `--adapter` 옵션을 설정합니다.
   ...
   (예: `--adapter json`)
21. GitHub API 인증 토큰을 사용하여 결과를 GitHub Check 및 Pull Request 주석으로 게시하기 위해 `--github-actions` 옵션을 설정합니다
...
[hash option]: /ko/docs/explanation/branches/#--hash-hash
