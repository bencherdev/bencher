...
1. 创建一个GitHub Actions `workflow` 文件。 (例如：`.github/workflows/benchmark.yml`)
...
5. API令牌必须已经存在。将`BENCHER_API_TOKEN`添加为**仓库**秘密。 (例如： `Repo -> Settings -> Secrets and variables -> Actions -> New repository secret`) 设置`--token`标志或`BENCHER_API_TOKEN`环境变量为API令牌。(例如：`BENCHER_API_TOKEN: ${{ secrets.BENCHER_API_TOKEN }}`)
...
7. 可选：设置 `--adapter` 参数或`BENCHER_ADAPTER` 环境变量为想要的adapter名字。 (例如：`BENCHER_ADAPTER: json`) 如果没有设置，则会使用`magic`适配器。查看 [基准测试适配器](/docs/zh/explanation/adapters)以获取完全概览。
...
10. 通过 <code><a href="/docs/zh/explanation/bencher-run">bencher run</a></code> CLI子命令 [跟踪你的基准测试](/docs/zh/how-to/track-benchmarks)：
    1.  可选：设置 `--branch` 标志或`BENCHER_BRANCH`环境变量为Branch的slug或UUID。（例如：`--branch main`）Branch必须事先存在。如果没有设置，那么`main` Branch会被默认使用。
    2.  如果生成Alert，设置命令为失败。（例如：`--err`）Alert的生成需要事先存在一个[阈值](/docs/zh/explanation/thresholds)。
    3.  运行你的基准测试并从结果中生成报告。 (例如：`"bencher mock"`)
...
1. 创建一个GitHub Actions `workflow` 文件。(例如：`.github/workflows/benchmark.yml`)
1. 在对`main`分支的`push`事件和`pull_request` 事件上运行。限制在`push`上**仅**对选定的分支（例如：`main`）运行非常重要，以防止对PR分支的push运行两次。
2. 不要总是使用`main` 分支，使用[GitHub Actions 默认的环境变量](https://docs.github.com/zh/actions/learn-github-actions/variables#default-environment-variables)：
        1. 如果已经存在，使用当前分支的数据。（例如：`--if-branch '${{ github.head_ref }}'`）
        2. 如果PR目标分支的数据和阈值已经存在，创建一个克隆。（例如：`--else-if-branch '${{ github.base_ref }}'`）
        3. 否则，创建`main`分支数据和阈值的克隆。（例如：`--else-if-branch main`）
        4. 设置项目分支有几个选项，查看 [分支选择](/docs/zh/explanation/branch-selection) 以获取完全概览。
3. 设置Github API认证令牌。（例如：`--github-actions "${{ secrets.GITHUB_TOKEN }}"`）当这个选项作为pull请求的一部分被设置时，结果将会作为一个评论添加到pull请求。这使用[GitHub Actions `GITHUB_TOKEN`环境变量](https://docs.github.com/zh/actions/security-guides/automatic-token-authentication).
4. 查看下面的[bencher run](/docs/zh/explanation/bencher-run)以获取完全概览. 配置pull请求评论有很多种方式，可以通过`--ci-*`参数来进行配置。
...
1. 创建一个GitHub Actions `workflow` 文件。(例如：`.github/workflows/pr_benchmark.yml`)
...
1. （未选示）创建第二个GitHub Actions `workflow`文件并使用初始示例在`main`分支的`push`事件上运行。(例如：`.github/workflows/benchmark.yml`)
...
1. 创建一个第二个工作流程文件，`Track Benchmarks`。
...
1. （未选示）创建第三个GitHub Actions `workflow`文件并使用初始示例在`main`分支的`push`事件上运行。(例如：`.github/workflows/benchmark.yml`)
