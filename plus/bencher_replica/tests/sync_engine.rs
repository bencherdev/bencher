...
mod cases {
    ...
    use bencher_replica::{
        CheckpointOutcome, EngineState, GenerationId, ReplicaMeta, Replicator, SnapshotStatus,
        VerifyReport, WalScanner, decompress_segment,
    };
    use bytes::Bytes;
    ...
    /// Seed the shipped-through-checkpoint meta proof, then leave an
    /// unshipped commit in a restarted WAL cycle: the burial precondition
    /// for the `final_sync` incomplete-drain tests.
    async fn seed_checkpoint_proof_with_unshipped_tail(harness: &mut Harness) {
        harness.ready().await;
        harness
            .fixture
        ...
        assert!(
            ...
            "the completed checkpoint seals the epoch in the meta"
        );
        // The WAL is fully backfilled, so this write restarts it; the
        // commit never ships.
        harness
            .fixture
            .txn(&["INSERT INTO t (data) VALUES ('never ships')"])
            .expect("unshipped txn");
    }

    /// An incomplete shutdown drain (ship failure) must revoke the meta's
    /// shipped-through-checkpoint proof: `SQLite`'s close-time checkpoint
    /// may backfill the unshipped frames and delete the WAL after
    /// `final_sync` returns, and a surviving proof would let the next boot
    /// continue the lineage with those frames silently buried.
    #[tokio::test]
    async fn final_sync_ship_failure_revokes_meta_checkpoint_proof() {
        let mut harness = Harness::new().await;
        seed_checkpoint_proof_with_unshipped_tail(&mut harness).await;
        harness
            .flaky()
            .set_plan(FailurePlan::new().fail_all(OpKind::Put));
        harness
            .engine
            .final_sync(Duration::from_secs(30))
            .await
            .expect("final sync");
        let meta = ReplicaMeta::load(&harness.fixture.db_path())
            .expect("load meta")
        ...
        assert!(
            !meta.epoch_shipped_through_checkpoint,
            "an incomplete drain revokes the proof so a WAL-less next boot \
             re-snapshots instead of trusting a buried tail"
        );
    }

    /// The deadline-elapsed drain branch revokes the proof exactly like the
    /// ship-failure branch. A zero deadline elapses at the drain's first
    /// await, before any segment ships.
    #[tokio::test]
    async fn final_sync_deadline_revokes_meta_checkpoint_proof() {
        let mut harness = Harness::new().await;
        seed_checkpoint_proof_with_unshipped_tail(&mut harness).await;
        harness
            .engine
        ...
        assert!(
            !meta.epoch_shipped_through_checkpoint,
            "the deadline branch revokes the proof like the ship-failure \
             branch"
        );
    }
    ...
    /// Impose a small per-transaction cap (there is no JSON knob, so mutate
    /// the resolved config and rebuild the engine), commit a transaction far
    /// over it, and drive the forced re-snapshot to completion, leaving epoch
    /// 0 pinned at the boundary (the oversized drain awaiting the WAL restart)
    /// with the drain checkpoint still to run. Returns the fresh generation
    /// that captured the oversized transaction.
    async fn oversized_pin_at_boundary(harness: &mut Harness) -> GenerationId {
        harness.ready().await;
        // The cap is above a normal single-page write (~4 KiB) but well below
        // the deliberately oversized transaction below.
    ...
    async fn oversized_committed_txn_re_snapshots_and_drains() {
        let mut harness = Harness::new().await;
        oversized_pin_at_boundary(&mut harness).await;
    ...
    /// The oversized-drain wedge (review finding F1): a commit landing between
    /// the boundary read and the drain checkpoint means the checkpoint gate
    /// defers on frames that can never ship (the ship path ships nothing while
    /// epoch 0 is pinned), so without escalation the engine stays wedged until
    /// the 24h snapshot interval. The mitigation escalates a stalled drain to
    /// a FRESH oversized re-snapshot (whose boundary is re-read past the
    /// wedging frames) on the OVERSIZED retrigger cadence (1h), and the drain
    /// then converges on the first quiet window between the boundary read and
    /// the drain checkpoint.
    #[tokio::test]
    async fn oversized_drain_stall_escalates_to_resnapshot_on_retrigger_cadence() {
        let mut harness = Harness::new().await;
        let wedged_gen = oversized_pin_at_boundary(&mut harness).await;

        // The wedge: commits keep landing past the pinned boundary, so every
        // drain checkpoint defers and nothing ships. Within the retrigger
        // window the rate limit holds: no snapshot churn per evaluation.
        for round in 0..4 {
            harness
                .fixture
                .txn(&["INSERT INTO t (data) VALUES ('wedge')"])
                .expect("wedge txn");
            harness.advance(60);
            let progress = harness.engine.sync_once().await.expect("wedged tick");
            assert_eq!(
                progress.shipped_segments, 0,
                "the pinned drain ships nothing (round {round}): {progress:?}"
            );
            // The drain checkpoint keeps deferring (driven directly to bypass
            // the min-pages threshold for this small fixture).
            let outcome = harness
                .engine
                .checkpoint_once()
                .await
                .expect("stalled checkpoint");
            assert_eq!(
                outcome,
                CheckpointOutcome::SkippedUnshipped,
                "the gate defers on the wedging frames (round {round})"
            );
            assert_eq!(
                harness.engine.generation(),
                Some(&wedged_gen),
                "no re-snapshot inside the retrigger window (round {round})"
            );
        }

        // Past the retrigger window (still far below the 24h snapshot
        // interval), the next stalled drain evaluation escalates to a fresh
        // re-snapshot instead of staying wedged.
        ...
        let outcome = harness
            ...
            .await
            .expect("stalled checkpoint");
        assert_eq!(outcome, CheckpointOutcome::SkippedUnshipped);
        for _ in 0..256 {
            if harness.engine.state() == EngineState::Streaming {
                break;
            }
            harness
                ...
                .sync_once()
                .await
                .expect("escalated re-snapshot tick");
        }
        let escalated_gen = harness.engine.generation().cloned().expect("generation");
        assert_ne!(
            escalated_gen, wedged_gen,
            "the stalled drain escalated to a fresh generation on the retrigger cadence"
        );

        // Quiet window: no commit lands before this drain checkpoint, so it
        // completes, and the next write restarts the WAL and shipping resumes.
        let outcome = harness
            .engine
            .checkpoint_once()
            .await
        ...
        assert_eq!(
            outcome,
            CheckpointOutcome::Completed,
            "the escalated drain completes once no commit lands in the window"
        );
        harness
            .fixture
            .txn(&["INSERT INTO t (data) VALUES ('after drain')"])
        ...
        assert!(
            progress.error.is_none() && progress.shipped_segments >= 1,
            "shipping resumes on a fresh cycle after the escalated drain: {progress:?}"
        );
        harness.assert_restore_equivalent().await;
    }

    /// Every stalled oversized-drain checkpoint evaluation is counted (the
    /// `replica.oversized.stalled` otel counter mirrors the engine-side count
    /// asserted here, and the error log fires on the same path), even while
    /// the re-snapshot escalation is rate-limited, so operators see the wedge
    /// immediately rather than at the escalation.
    #[tokio::test]
    async fn oversized_drain_stall_counts_each_stalled_evaluation() {
        let mut harness = Harness::new().await;
        let wedged_gen = oversized_pin_at_boundary(&mut harness).await;
        assert_eq!(
            harness.engine.oversized_stalled_count(),
            0,
            "no stall before a commit lands past the boundary"
        );
        // One wedging commit past the boundary stalls every subsequent drain
        // checkpoint.
        harness
            .fixture
            .txn(&["INSERT INTO t (data) VALUES ('wedge')"])
        ...
        for expected in 1..=3u64 {
            ...
            assert_eq!(
                harness.engine.oversized_stalled_count(),
                expected,
                "each stalled evaluation counts"
            );
            assert_eq!(
                harness.engine.generation(),
                Some(&wedged_gen),
                "the rate limit holds: counting a stall does not churn a snapshot"
            );
        }
    }
}
