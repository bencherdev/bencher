...
1. オプション：`--testbed`フラグまたは`BENCHER_TESTBED`環境変数をTestbedのスラッグまたはUUIDに設定します。Testbedは既に存在している必要があります。これが設定されていない場合、デフォルトの`localhost` Testbedが使用されます。(例：`--testbed localhost`)
