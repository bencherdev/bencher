...
[クイックスタート][quickstart]および[Docker 自己ホスト][docker self-hosted]チュートリアルから引き継いで、`claimed`プロジェクトに相対[連続ベンチマーク][continuous benchmarking]を追加してみましょう。

> 🐰 続ける前に、
> [APIトークンを作成して、`BENCHER_API_KEY`環境変数として設定します][create an api token]
>！
