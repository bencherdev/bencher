...
Por razones de seguridad, secretos como tu `BENCHER_API_KEY` y el `GITHUB_TOKEN` no están disponibles en GitHub Actions para PRs desde forks.
...
No hay secretos como tu `BENCHER_API_KEY` y el `GITHUB_TOKEN` disponibles allí.
...
donde los secretos como tu `BENCHER_API_KEY` y el `GITHUB_TOKEN` están disponibles.
