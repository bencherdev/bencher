...
C++ Googleアダプター(`cpp_google`)は、[Google Benchmark](https://github.com/google/benchmark)の[JSONフォーマットの出力（つまり `--benchmark_format=json`）](https://github.com/google/benchmark/blob/main/docs/ja/user_guide.md#output-formats)を期待します。
`latency` Metric Kind（つまり `nanoseconds (ns)`）が収集されます。
...
JavaScript Timeアダプター(`js_time`)は、[console.time](https://developer.mozilla.org/en-US/docs/ja/Web/API/console/time)/[console.timeEnd](https://developer.mozilla.org/en-US/docs/ja/Web/API/console/timeEnd)の出力を期待します。
`latency` Metric Kind（つまり `nanoseconds (ns)`）が収集されます。
