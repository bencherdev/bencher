...
Hay dos esquemas de replicación:
...
Ambos esquemas de réplica tienen las siguientes opciones adicionales:
...
- `snapshot.retention`: (Opcional) Cantidad de tiempo que se conservarán los archivos de snapshot y WAL. Después del período de retención, se creará una nueva instantánea y se eliminará la antigua. Los archivos WAL que existan antes de la instantánea más antigua también se eliminarán. Por defecto es `24h`.
- `validation.interval`: (Opcional) Cuando se especifica, Bencher restaurará automáticamente y validará que los datos en la réplica coincidan con la copia local. Deshabilitado por defecto. Habilitar esto puede aumentar significativamente el costo de ejecutar Bencher, ya que la mayoría de los servicios en la nube cobran por las descargas.
...
|      snapshot.interval       |   "1h"    |     ---     |    No     | Con qué frecuencia se crean nuevas instantáneas. Reduce el tiempo de restauración. |
|      snapshot.retention      |  "24h"    |     24h     |    No     | Cuánto tiempo se conservan los archivos de snapshot y WAL antes de ser reemplazados. |
|     validation.interval      |   "6h"    |     ---     |    No     | Con qué frecuencia se restauran y validan los datos de réplica contra la copia local. Deshabilitado por defecto. |
