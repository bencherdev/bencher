...
impl DataStore {
    pub async fn backup(&self, source_path: &Path, file_name: &str) -> Result<(), DatabaseError> {
        match self {
...
impl AwsS3 {
    fn new(
        ...
        let (region, accesspoint_arn) = access_point
            ...
            .split_once(COLON)
            .ok_or_else(|| DatabaseError::DataStore(access_point.to_owned()))?;
    ...
    async fn backup(&self, source_path: &Path, file_name: &str) -> Result<(), DatabaseError> {
        ...
        let body = aws_sdk_s3::primitives::ByteStream::from_path(source_path)
            .await
            .map_err(|e| DatabaseError::AwsS3(e.to_string()))?;
