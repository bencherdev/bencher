...
8. Enregistrez votre code source. (ex: `uses: actions/checkout@v4`)
...
3. Définissez [les permissions pour le `GITHUB_TOKEN`](https://docs.github.com/fr/actions/using-jobs/assigning-permissions-to-jobs#setting-the-github_token-permissions-for-a-specific-job) sur `write` pour les `pull-requests`. Selon vos paramètres GitHub, cela peut ne pas être nécessaire. Cependant, pour toutes les organisations et les dépôts personnels [créés après le 02 février 2023](https://github.blog/changelog/2023-02-02-github-actions-updating-the-default-github_token-permissions-to-read-only/), c'est le comportement par défaut. Consultez [la documentation GitHub](https://docs.github.com/fr/actions/security-guides/automatic-token-authentication#modifying-the-permissions-for-the-github_token) pour un aperçu complet.
4. [Sélectionnez la branche](/fr/docs/explanation/branch-selection/) à utiliser :
...
5. Définissez le jeton d'authentification de l'API GitHub. (ex : `--github-actions "${{ secrets.GITHUB_TOKEN }}"`) Lorsque cette option est définie dans le cadre d'une pull request, les résultats seront ajoutés à la pull request sous forme de commentaire. Cela utilise [la variable d'environnement `GITHUB_TOKEN` des GitHub Actions](https://docs.github.com/fr/actions/security-guides/automatic-token-authentication).
6. Consultez la documentation [bencher run](/fr/docs/explanation/bencher-run/) pour un aperçu complet de toutes les façons de configurer le commentaire de la pull request avec les drapeaux `--ci-*`.
7. (Non montré) Créez un second fichier `workflow` pour les GitHub Actions et utilisez l'exemple initial ci-dessus pour exécuter lors des événements `push` vers la branche `main`. (ex : `.github/workflows/benchmarks.yml`)
