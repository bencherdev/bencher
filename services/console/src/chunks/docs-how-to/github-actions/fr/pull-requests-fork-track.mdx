...
6. Définissez [les permissions pour le `GITHUB_TOKEN`][github token permissions]
   sur `write` pour `checks` et `pull-requests`.
...
14. Définissez l'option `--key` sur le secret **Repository** `BENCHER_API_KEY`.
    Consultez les [documents `--key`][key option] pour plus de détails.
...
19. Définissez le drapeau `--error-on-alert` pour échouer la commande si une alerte est générée.
...
20. Définissez l'option `--adapter` sur le format JSON du Bencher Metric Format (`json`) généré par <code><a href="/fr/docs/reference/bencher-metric-format/#bencher-mock">bencher mock</a></code>.
   ...
   (ex: `--adapter json`)
21. Définissez l'option `--github-actions` sur le jeton d'authentification de l'API GitHub pour publier les résultats sous forme de vérification GitHub et de commentaire sur la Pull Request en utilisant
...
[hash option]: /fr/docs/explanation/branches/#--hash-hash
