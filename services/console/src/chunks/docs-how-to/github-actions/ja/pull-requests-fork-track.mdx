...
6. `GITHUB_TOKEN` の[権限を設定][github token permissions]し、`checks` と `pull-requests` 用に `write` に設定します。
...
14. `--key` オプションを `BENCHER_API_KEY` **リポジトリ**シークレットに設定します。
    詳細は、[`--key` ドキュメント][key option]をご覧ください。
...
19. アラートが生成された場合にコマンドを失敗させる `--error-on-alert` フラグを設定します。
   完全な概要については、[`--error-on-alert` ドキュメント][alert err]を参照してください。
   (例: `--error-on-alert`)
20. <code><a href="/ja/docs/reference/bencher-metric-format/#bencher-mock">bencher mock</a></code>によって生成された[Bencher Metric Format JSON (`json`)][bmf] に `--adapter` オプションを設定します。
   ...
   (例: `--adapter json`)
21. [GitHub Actions `GITHUB_TOKEN` 環境変数][github token]を使用して GitHub チェックおよびプルリクエストへのコメントとして結果を投稿するために、GitHub API 認証トークンに `--github-actions` オプションを設定します。
...
[hash option]: /ja/docs/explanation/branches/#--hash-hash
[start point]: /ja/docs/explanation/branches/#--start-point-branch
[start point hash]: /ja/docs/explanation/branches/#--start-point-hash-hash
[start point clone thresholds]: /ja/docs/explanation/branches/#--start-point-clone-thresholds
