...
///
/// List the SSO domains for an organization.
/// The user must have `view_role` permissions for the organization.
/// By default, the domains are sorted in alphabetical order.
/// The HTTP response header `X-Total-Count` contains the total number of domains.
#[endpoint {
    method = GET,
    path =  "/v0/organizations/{organization}/sso",
    tags = ["organizations", "sso"]
