...
use crate::segment::{MAX_DECOMPRESSED_BYTES, decompress_segment_with_cap};
...
fn plan_epochs(
    ...
    // A retried ship whose earlier PUT persisted server-side but surfaced as
    // an error can leave two segments with the same (epoch, salt, start) and
    // different ends. Both were scanned from the same verified
    // (offset, checksum) seed over the same immutable epoch bytes, so the
    // shorter is a byte prefix of the longer: keep only the largest end (the
    // sort puts it last within a run) and let the full checksum-chain
    // pre-validation of the assembled WAL remain the backstop. Duplicates
    // under DIFFERENT salts are a lineage collision, not a retry, and are
    // left for the salt check below to discard.
    segments.dedup_by(|later, earlier| {
        let duplicate = later.epoch == earlier.epoch
            && later.salt == earlier.salt
            && later.start == earlier.start;
        if duplicate {
            // `dedup_by` removes `later` when true; the later element has
            // the larger end, so promote it into the retained slot.
            *earlier = *later;
        }
        duplicate
    });
...
async fn build_epoch_wal(
    ...
    for segment in &plan.segments {
        ...
        let cap = segment_decompression_cap(expected);
        let raw = spawn_blocking(move || decompress_segment_with_cap(&compressed, cap))
            .await
            .map_err(RestoreError::Join)?;
        let raw = match raw {
            Ok(raw) => raw,
...
/// The decompression cap for one segment: the exact raw size its key claims,
/// plus one byte to catch an overrun. The key encodes the size, so a corrupt
/// or hostile segment fails immediately instead of inflating toward the
/// generic multi-gigabyte ceiling before the length check rejects it; and
/// because the key itself is attacker-writable data, the cap is clamped to
/// [`MAX_DECOMPRESSED_BYTES`] so a key claiming an absurd byte range cannot
/// authorize a larger allocation than any legitimate segment could need.
fn segment_decompression_cap(expected: u64) -> u64 {
    expected.saturating_add(1).min(MAX_DECOMPRESSED_BYTES)
}
...
mod tests {
    ...
    use super::{
        ...
        segment_decompression_cap, validate_epoch_wal_blocking, with_retry,
    };
    use crate::backoff::Backoff;
    use crate::position::SegmentKey;
    use crate::segment::MAX_DECOMPRESSED_BYTES;
    ...
    #[test]
    fn plan_dedupes_duplicate_start_keeping_largest_end() {
        // A retried ship after an ambiguous PUT success (persisted
        // server-side, surfaced as an error) leaves two segments with the
        // same start and different ends; the shorter is a byte prefix of the
        // longer. Only the largest end survives, and the chain stays
        // contiguous through it.
        let segments = vec![seg(0, 0, 32), seg(0, 32, 64), seg(0, 32, 128)];
        let (plans, violation) = plan_epochs(segments, 0, None);
        assert_eq!(
            violation, None,
            "a duplicate-start retry orphan is not a violation"
        );
        assert_eq!(plan_shape(&plans), vec![(0, 2)]);
        assert_eq!(
            plans[0].segments,
            vec![seg(0, 0, 32), seg(0, 32, 128)],
            "the shorter duplicate is dropped, the largest end replays"
        );
    }

    #[test]
    fn plan_does_not_dedupe_duplicate_start_across_salts() {
        // The same start under DIFFERENT salts is a lineage collision, not a
        // retry duplicate: the salt check must still discard the epoch.
        let segments = vec![
            seg(0, 0, 32),
            seg(0, 32, 64),
            seg_with_salt(0, OTHER_SALT, 32, 128),
        ];
        let (plans, violation) = plan_epochs(segments, 0, None);
        assert_eq!(violation, Some(seg_with_salt(0, OTHER_SALT, 32, 128)));
        assert_eq!(plan_shape(&plans), Vec::<(u64, usize)>::new());
    }

    #[test]
    fn segment_decompression_cap_clamps_hostile_key_ranges() {
        // A legitimate key authorizes exactly its claimed size plus the
        // one-byte overrun sentinel.
        assert_eq!(segment_decompression_cap(4096), 4097);
        assert_eq!(segment_decompression_cap(0), 1);
        // The key is attacker-writable data: a claimed range beyond the
        // crate-wide ceiling must not authorize a larger allocation.
        assert_eq!(
            segment_decompression_cap(MAX_DECOMPRESSED_BYTES),
            MAX_DECOMPRESSED_BYTES
        );
        assert_eq!(
            segment_decompression_cap(u64::MAX),
            MAX_DECOMPRESSED_BYTES,
            "an absurd claimed range clamps to the generic ceiling"
        );
    }
