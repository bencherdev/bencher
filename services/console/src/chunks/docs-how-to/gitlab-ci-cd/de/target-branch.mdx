...
8. Setzen Sie die `--key`-Option auf die maskierte `BENCHER_API_KEY` Umgebungsvariable.
...
12. Setzen Sie das `--error-on-alert`-Flag, um den Befehl zu beenden, wenn eine Warnung generiert wird.
