...
The Python ASV Adapter (`python_asv`) expects [airspeed velocity](https://github.com/airspeed-velocity/asv) output.
The `latency` metric kind (ie `nanoseconds (ns)`) is gathered.
The `lower_bound` and `upper_bound` are the standard deviation below and above the median (ie `value`) respectively.

## 🐍 Python Pytest

The Python Pytest Adapter (`python_pytest`) expects [pytest-benchmark](https://github.com/ionelmc/pytest-benchmark) output.
The `latency` metric kind (ie `nanoseconds (ns)`) is gathered.
The `lower_bound` and `upper_bound` are the standard deviation below and above the median (ie `value`) respectively.
