...
et sinon marqué `unknown` (le contact avec le Runner a été perdu).
...
La limite court à partir du démarrage du Job, ou de sa réclamation s'il n'a jamais démarré, et s'applique aussi à un Job `unknown`.
...
sinon, le Job est marqué `unknown`, ou `canceled` s'il avait dépassé le délai d'attente strict.
Le serveur vérifie à nouveau un Job `unknown` à son échéance.
Au démarrage, le serveur marque également les Jobs `claimed` orphelins comme `unknown`,
replanifie les délais d'attente des Jobs en cours et `unknown`,
et retraite les Jobs `completed` dont les résultats ont été stockés mais pas encore analysés ;
un Job `completed` dont les résultats ont été perdus est marqué `unknown` afin que le Runner puisse les renvoyer.
...
Le résultat d'un Runner résout un Job `unknown`.
Un Job `failed` ou `canceled` ne change plus jamais d'état : un résultat tardif est acquitté, pour que le Runner cesse de le renvoyer, puis ignoré.
