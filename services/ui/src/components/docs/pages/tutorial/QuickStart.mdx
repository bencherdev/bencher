...
Bencher is a suite of open source [continuous benchmarking](/docs/explanation/continuous-benchmarking) tools designed to help catch performance regressions in CI. That is, Bencher allows you to detect and prevent performance regressions _before_ they make it to production.
...
> 🐰 Note: The `bencher` CLI defaults to [https://api.bencher.dev](https://api.bencher.dev) as the host, so when using Bencher Cloud this step is not required.
