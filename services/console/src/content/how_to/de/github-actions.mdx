...
10. [Verfolgen Sie Ihre Benchmarks](/docs/de/how-to/track-benchmarks) mit dem <code><a href="/docs/de/explanation/bencher-run">bencher run</a></code> CLI-Unterbefehl:
    ...
    2.  Lassen Sie den Befehl fehlschlagen, wenn ein Alert generiert wird. (z.B.: `--err`). Damit ein Alert generiert werden kann, muss bereits ein [Schwellenwert](/docs/de/explanation/thresholds) existieren.
...
1. Erstellen Sie eine GitHub Actions `workflow` Datei. (z.B.: `.github/workflows/benchmark.yml`)
2. Führen Sie bei `push` Ereignissen zum `main` branch und bei `pull_request` Ereignissen aus. Es ist wichtig, den run bei `push` **nur** auf die ausgewählten Branches (z.B.: `main`) zu begrenzen, um zu verhindern, dass Pushes zu PR-Branches doppelt ausgeführt werden!
3. Anstatt immer den `main` Branch zu verwenden, verwenden Sie die [GitHub Action Standardumgebungsvariablen](https://docs.github.com/de/actions/learn-github-actions/variables#default-environment-variables) um:
        1. Verwenden Sie die aktuellen Branch-Daten, falls diese bereits existieren. (z.B.: `--if-branch '${{ github.head_ref }}'`)
        2. Erstellen Sie eine Kopie der PR-Zielbranch-Daten und -Schwellenwerte, falls diese bereits existieren. (z.B.: `--else-if-branch '${{ github.base_ref }}'`)
        3. Ansonsten erstellen Sie eine Kopie der `main` Branch-Daten und -Schwellenwerte. (z.B.: `--else-if-branch main`)
        4. Es gibt mehrere Optionen für die Auswahl des Projekt-Branches. Siehe [Branch Auswahl](/docs/de/explanation/branch-selection) für einen vollständigen Überblick.
4. Setzen Sie den GitHub API-Authentifizierungstoken. (z.B.: `--github-actions "${{ secrets.GITHUB_TOKEN }}"`). Wenn diese Option als Teil einer Pull-Anfrage gesetzt ist, werden die Ergebnisse als Kommentar zur Pull-Anfrage hinzugefügt. Hierbei wird [die GitHub Actions `GITHUB_TOKEN` Umgebungsvariable](https://docs.github.com/de/actions/security-guides/automatic-token-authentication) verwendet.
5. Siehe die [bencher run](/docs/de/explanation/bencher-run) Dokumentation für einen vollständigen Überblick über alle Möglichkeiten zur Konfiguration des Kommentars zur Pull-Anfrage mit den `--ci-*` Optionen.
...
1. Erstellen Sie eine GitHub Actions `workflow` Datei. (z.B.: `.github/workflows/pr_benchmark.yml`)
...
5. (Nicht dargestellt) Erstellen Sie eine zweite GitHub Actions `workflow` Datei und verwenden Sie das obige erste Beispiel, um bei `push` Ereignissen zum `main` Branch zu laufen. (z.B.: `.github/workflows/benchmark.yml`)
...
7. (Nicht dargestellt) Erstellen Sie eine dritte GitHub Actions `workflow` Datei und verwenden Sie das obige erste Beispiel, um bei `push` Ereignissen zum `main` Branch zu laufen. (z.B.: `.github/workflows/benchmark.yml`)
