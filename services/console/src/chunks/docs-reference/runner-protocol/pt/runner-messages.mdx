...
| ----------- | ------------------------------------------------- | ----------------------------------------------------------------------- |
| `ready`     | O Runner está ocioso e solicitando um Job         | `poll_timeout` opcional (1-900s) e metadados do `runner` (`os`, `arch`, `version`, `channel` de atualização opcional e `checksum` do binário opcional) |
