...
你已经用 [`--branch` option][branch option] 指定了一个 Branch，
并用 [`--testbed` option][testbed option] 指定了一个 Testbed。
...
> 可以通过 [`--start-point-clone-thresholds` flag][start point clone thresholds flag] 实现。
