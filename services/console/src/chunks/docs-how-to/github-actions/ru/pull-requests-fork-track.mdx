...
6. Установите [разрешения для `GITHUB_TOKEN`][github token permissions]
   ...
   (например, `permissions: checks: write pull-requests: write`)
...
14. Установите параметр `--key` в значение секрета **Repository** `BENCHER_API_KEY`.
    Подробности смотрите в [документации `--key`][key option].
    (например, `--key '${{ secrets.BENCHER_API_KEY }}'`)
...
19. Установите флаг `--error-on-alert`, чтобы команда завершилась неудачно, если сгенерируется Уведомление.
   Полный обзор см. в [документации `--error-on-alert`][alert err].
...
21. Установите параметр `--github-actions` на значение токена аутентификации GitHub API для публикации результатов в виде проверки GitHub и комментария к Pull Request, используя
...
[hash option]: /ru/docs/explanation/branches/#--hash-hash
[start point]: /ru/docs/explanation/branches/#--start-point-branch
[start point hash]: /ru/docs/explanation/branches/#--start-point-hash-hash
[start point clone thresholds]: /ru/docs/explanation/branches/#--start-point-clone-thresholds
[start point reset]: /ru/docs/explanation/branches/#--start-point-reset
