...
// This document was automatically generated by OpenAI GPT-4/5. It might not be accurate and might contain errors. If you find any errors, please <>open an issue on GitHub</>.
...
const DisclosureDe = (props: { page: string }) => {
	return (
		<DisclosureInner
			bodyText="Dieses Dokument wurde automatisch von OpenAI GPT-4/5 generiert. Es ist möglicherweise nicht korrekt und kann Fehler enthalten. Wenn Sie Fehler finden, "
...
const DisclosureEs = (props: { page: string }) => {
	return (
		<DisclosureInner
			bodyText="Este documento fue generado automáticamente por OpenAI GPT-4/5. Puede que no sea exacto y contenga errores. Si encuentra algún error, "
...
const DisclosureFr = (props: { page: string }) => {
	return (
		<DisclosureInner
			bodyText="Ce document a été automatiquement généré par OpenAI GPT-4/5. Il peut ne pas être précis et peut contenir des erreurs. Si vous trouvez des erreurs, veuillez "
...
const DisclosureJa = (props: { page: string }) => {
	return (
		<DisclosureInner
			bodyText="このドキュメントは OpenAI GPT-4/5 によって自動的に生成されました。 正確ではない可能性があり、間違いが含まれている可能性があります。 エラーを見つけた場合は、"
...
const DisclosurePt = (props: { page: string }) => {
	return (
		<DisclosureInner
			bodyText="Este documento foi gerado automaticamente pelo OpenAI GPT-4/5. Pode não ser preciso e pode conter erros. Se você encontrar algum erro, "
...
const DisclosureRu = (props: { page: string }) => {
	return (
		<DisclosureInner
			bodyText="Этот документ был автоматически создан OpenAI GPT-4/5. Оно может быть неточным и содержать ошибки. Если вы обнаружите какие-либо ошибки, "
...
const DisclosureZh = (props: { page: string }) => {
	return (
		<DisclosureInner
			bodyText="该文档由 OpenAI GPT-4/5 自动生成。 它可能不准确并且可能包含错误。 如果您发现任何错误，请"
