...
8. 将 `--key` 选项设置为已屏蔽的 `BENCHER_API_KEY` 环境变量。
   查看 [`--key` 文档][key option] 以获取更多详情。
   （例如：`--key "$BENCHER_API_KEY"`）
9.  将 `--branch` 选项设置为分支名称。
   查看 [`--branch` 文档][branch option] 以获取完整概述。
   （例如：`--branch main`）
10. 将 `--testbed` 选项设置为测试平台名称。
    这可能应与在 `image` 中选择的机器匹配。
    查看 [`--testbed` 文档][testbed option] 以获取更多详情。
...
11. 为 `main` 分支、`debian:bullseye` 测试平台和 `latency` 测量设置阈值：
    ...
    查看 [`--threshold-measure` 文档][threshold measure option] 以获取更多详情。
    （例如：`--threshold-measure latency`）
    2. 将 `--threshold-test` 选项设置为 Student's t-test (`t_test`)。
    查看 [`--threshold-test` 文档][threshold test option] 以获取完整概述。
    （例如：`--threshold-test t_test`）
    3. 将 `--threshold-max-sample-size` 选项设置为最大样本大小 `64`。
    查看 [`--threshold-max-sample-size` 文档][threshold max sample size] 以获取更多详情。
    （例如：`--threshold-max-sample-size 64`）
    4. 将 `--threshold-upper-boundary` 选项设置为上界 `0.99`。
    查看 [`--threshold-upper-boundary` 文档][threshold upper boundary] 以获取更多详情。
    （例如：`--threshold-upper-boundary 0.99`）
    5. 设置 `--thresholds-reset` 标志以确保只有指定的阈值处于活动状态。
    查看 [`--thresholds-reset` 文档][thresholds reset] 以获取完整概述。
    （例如：`--thresholds-reset`）
12. 设置 `--error-on-alert` 标志以在生成警报时使命令失败。
    查看 [`--error-on-alert` 文档][alert err] 以获取完整概述。
    （例如：`--error-on-alert`）
