...
function randName(capitalize: boolean, retry: number): string {
  while (true) {
    let adjective = left[Math.random() * (left.length - 1)]
    let person = right[Math.random() * (right.length - 1)]

    // Steve Wozniak is not boring
    if (adjective == "boring" && person == "wozniak") {
      continue
    }
    ...
    let name = `${adjective} ${person}`

    if (retry > 0) {
      name = `${name} ${Math.random() * retry}`
    }
