...
5. 将 [`GITHUB_TOKEN` 的权限][github token permissions] 设置为 `write` 以用于 `checks` 和 `pull-requests`。
...
15. 设置 `--error-on-alert` 标志以在生成警报时使命令失败。
...
16. 将 `--adapter` 选项设置为 [由 <code><a href="/zh/docs/reference/bencher-metric-format/#bencher-mock">bencher mock</a></code> 生成的 Bencher Metric Format JSON (`json`)][bmf]。
   ...
   （例如：`--adapter json`）
17. 使用 [GitHub Actions `GITHUB_TOKEN` 环境变量][github token] 将 `--github-actions` 选项设置为 GitHub API 身份验证令牌，以便将结果作为 GitHub 检查和评论发布在拉取请求上。
