...
10. Establece la opción `--key` al secreto de **Repositorio** `BENCHER_API_KEY`. Consulta [la documentación de `--key`][key option] para más detalles.
...
13. Establece el Umbral para la Rama `main`, Testbed `ubuntu-latest` y la Medida `latency`:
    ...
    (ej: `--thresholds-reset`)
14. Establece la bandera `--error-on-alert` para que el comando falle si se genera una Alerta. Consulta [la documentación de `--error-on-alert`][alert err] para una visión completa.
   (ej: `--error-on-alert`)
