...
3. [Wählen Sie den Branch](http://localhost:3000/docs/de/explanation/branch-selection) aus, den Sie verwenden möchten:
        1. Verwenden Sie die aktuellen Branch-Daten, wenn sie bereits existieren. (z.B.: `--if-branch '${{ github.head_ref }}'`)
        2. Erstellen Sie eine Kopie der PR-Zielbranch-Daten und -Schwellenwerte, wenn sie bereits existieren. (z.B.: `--else-if-branch '${{ github.base_ref }}'`)
