...
1. Optionnel : Définissez le drapeau `--testbed` ou la variable d'environnement `BENCHER_TESTBED` sur le slug du Testbed ou l'UUID. Le Testbed doit déjà exister. Si cela n'est pas défini, alors le Testbed par défaut `localhost` sera utilisé. (ex: `--testbed localhost`)
