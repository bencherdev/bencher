...
import RustIaiCallgrind from "../../../chunks/explanation/ko/adapter-rust-iai-callgrind.mdx";
...
이것은 벤치마크 커맨드(ex: `bencher run --file results.json "bencher mock > results.json"`)와 벤치마크 커맨드 없이도 동작합니다(ex: `bencher mock > results.json && bencher run --file results.json`).
