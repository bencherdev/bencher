...
[Quickstart][quickstart] und [Docker Self-Hosted][docker self-hosted] Tutorials,
fügen wir Relatives [Continuous Benchmarking][continuous benchmarking] zu unserem `claimed` Projekt hinzu.

> 🐰 Stellen Sie sicher, dass Sie
> [einen API-Token erstellt und als `BENCHER_API_KEY` Umgebungsvariable gesetzt haben][create an api token]
...
7. Setzen Sie das `--error-on-alert` Flag, um den Befehl zu beenden, falls ein Alert erzeugt wird.
   Siehe [die `--error-on-alert` Dokus][alert err] für eine vollständige Übersicht.
   (z.B.: `--error-on-alert`)
