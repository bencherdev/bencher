...
//! loss (not merely atomically visible). A `put` or multipart `finish` that
//! fails removes its own partial file so a crash-looping writer cannot
//! accumulate them. Because
...
impl LocalMultipart {
    ...
    pub(crate) async fn finish(self) -> Result<(), StorageError> {
        ...
        // On any failure, remove the partial (mirroring `put`): the startup
        // sweep only runs at boot, so repeated finish failures under disk
        // pressure would otherwise accumulate multi-GB partials; the
        // original error wins.
        match sync_then_rename(file, &partial_path, &final_path).await {
            Ok(()) => Ok(()),
            Err(error) => {
                remove_partial(&partial_path).await;
                Err(error.into())
...
/// Fsync the already-written file, rename it into place, and fsync the
/// parent directory so the rename is durable. The single fallible region a
/// failing multipart `finish` guards with partial-file cleanup.
async fn sync_then_rename(
    file: fs::File,
    partial: &Utf8Path,
    final_path: &Utf8Path,
) -> Result<(), LocalError> {
...
/// Best-effort removal of a partial file on a `put` or multipart `finish`
/// error path. A failure
...
mod tests {
    ...
    #[tokio::test]
    async fn multipart_finish_error_removes_partial() {
        let tmp = tempfile::tempdir().expect("tempdir failed");
        let local = storage(&tmp);
        let mut upload = local
            .start_multipart("snap/db.zst")
        ...
        upload
            ...
            .await
            .expect("write failed");
        // Plant a directory at the final path so the rename fails AFTER the
        // partial file has been written, exercising the cleanup path.
        std::fs::create_dir_all(local.root().join("snap/db.zst").as_std_path())
            .expect("plant directory at key path");
        let error = upload
            .finish()
            .await
            .expect_err("finish onto a directory key must fail");
        assert!(
        ...
        assert!(
            partials.is_empty(),
            "a failed finish must leave no partial file behind, found: {partials:?}"
        );
    }
