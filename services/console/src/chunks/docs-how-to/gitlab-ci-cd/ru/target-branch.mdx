...
8. Установите опцию `--key` на маскированную переменную окружения `BENCHER_API_KEY`.
   Смотрите [документацию `--key`][key option] для получения подробной информации.
...
12. Установите флаг `--error-on-alert`, чтобы команда завершалась с ошибкой, если создаётся предупреждение.
   Смотрите [документацию `--error-on-alert`][alert err] для полного обзора.
