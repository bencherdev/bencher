...
9. Установите опцию `--key` для скрытой переменной окружения `BENCHER_API_KEY`.
   Смотрите [документацию по `--key`][key option] для получения более подробной информации.
