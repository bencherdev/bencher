...
[빠른 시작][quickstart] 및 [자체 호스팅 Docker][docker self-hosted] 튜토리얼에서 이어서, `claimed` 프로젝트에 상대적 [지속적 벤치마킹][continuous benchmarking]을 추가하겠습니다.

> 🐰 계속하기 전에
> [API 토큰을 생성하고 이를 `BENCHER_API_KEY` 환경 변수로 설정][create an api token]하십시오!
