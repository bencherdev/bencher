...
//! names everything a threshold checks: the variants, the measure, and the metric
...
//! the report creates checks the very report that created it, because thresholds are
...
///
/// The metric name of a threshold that checks the conventional name is `value`, and
/// the variants of a threshold that checks every one of them is `*`, so the two
...
// checked until the sixth report both declared the entries and carried the outliers.
