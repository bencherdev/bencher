...
查看[基准测试概述](/docs/zh/explanation/benchmarking)以便更深入地理解。
它们可以在<code><a href="/docs/zh/explanation/bencher-run">bencher run</a></code> CLI子命令中通过可选的`--adapter`标志进行指定。
...
`benchmark_name`对象包含Metric Kind别名或UUID作为键。
...
如果BMF JSON存储在文件中，
那么你可以使用<code><a href="/docs/zh/explanation/bencher-run">bencher run</a></code> CLI子命令并使用可选的`--file`参数来指定那个文件路径。
...
这可以在<code><a href="/docs/zh/explanation/bencher-run">bencher run</a></code> CLI子命令中通过可选的`--average`标志进行指定。
...
这可以在<code><a href="/docs/zh/explanation/bencher-run">bencher run</a></code> CLI子命令中通过可选的`--average`参数进行指定。
...
这可以在<code><a href="/docs/zh/explanation/bencher-run">bencher run</a></code> CLI 子命令中通过可选的 `--average` 标志来指定。
...
<h2><a href="/docs/zh/explanation/thresholds">继续前进：阈值和警告 ➡</a></h2>