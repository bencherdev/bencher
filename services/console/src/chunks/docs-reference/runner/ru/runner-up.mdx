...
Постоянный каталог состояния Runner.
Изолирующая среда (jail), ограничивающая песочницу, создаётся внутри этого каталога,
а среды, оставшиеся после аварийного завершения, удаляются из него перед запуском Job.
По умолчанию используется `/var/lib/bencher-runner`.
Также может быть задан переменной окружения `BENCHER_STATE_DIR`.
