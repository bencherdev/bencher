...
PR 브랜치 이름 (head ref)을 **단일** 인용부호로 감싸는 것도 _매우_ 중요합니다. (예: `'${{ github.head_ref }}'`) 그렇지 않으면 공격자가 명령 인젝션을 수행하는 악의적으로 이름 지은 브랜치를 만들 수 있습니다. 불신할 만한 입력에서 pwn 요청을 방지하기 위한 전체 개요는 이 [GitHub 보안 랩 소개](https://securitylab.github.com/research/github-actions-untrusted-input/)를 참조하십시오.
