## Pending `v0.6.7`
- Derive the project from `--image` for bare metal `bencher run` invocations; `--project` is now optional when `--image` includes the project (e.g. `registry.bencher.dev/<project>:<tag>` or `<project>:<tag>`)
