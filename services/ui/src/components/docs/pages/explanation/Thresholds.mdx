...
> 🐰 Tip: To fail a CI build when a boundary is violated use the `--err` flag for the <code><a href="/docs/reference/bencher-run">bencher run</a></code> CLI subcommand.
...
To fail a CI build in the event of an Alert set the `--err` flag when using the <code><a href="/docs/reference/bencher-run">bencher run</a></code> CLI subcommand.
