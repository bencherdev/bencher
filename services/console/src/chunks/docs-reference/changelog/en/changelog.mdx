## Pending `v0.6.13`
- Fix slow alert lists and perf queries on large projects
- Alert responses now name the `variant` the alert fired on
- **BREAKING CHANGE** Mark a Bare Metal job `unknown` instead of `failed` when the server loses contact with its runner
- Fix rate limit windows counting up to twice their length
- Write API server logs to stderr as JSON, one log record per line, so a logged value cannot forge a log line
- Add callbacks for detached Bare Metal jobs: `bencher run --image ... --detach --callback-url <URL>` has Bencher send one HTTP request when the job finishes (requires a Bencher Plus plan), `bencher run --job <UUID>` attaches to a submitted job and posts its results, and on GitHub Actions `--ci-callback-token` hands the GitHub Check to [a second workflow](/docs/how-to/github-actions/#pull-requests-on-bare-metal) (idea credit to [jkaczman](https://github.com/jkaczman): [#1046](https://github.com/bencherdev/bencher/issues/1046))
- **BREAKING CHANGE** `--detach` with `--github-actions` now requires `--ci-callback-token` or `--callback-url`, and no longer posts a comment or completes the GitHub Check before the job has run
- **BREAKING CHANGE** An empty `--github-actions` token, such as an unset secret, is now refused before the run starts, instead of being ignored outside GitHub Actions, only warning on an event other than a pull request, or failing a pull request's run after the results were uploaded
- Complete the GitHub Check as `failure` when a Bare Metal job fails and as `cancelled` when it is canceled, instead of `success`

## `0.6.12`
- Support Gungraun JSON format (Thank you [gamma0987](https://github.com/gamma0987))
...
- Add [Bencher MCP](/docs/how-to/mcp/), a hosted [Model Context Protocol](https://modelcontextprotocol.io) server at `https://mcp.bencher.dev/mcp` (`/mcp` on Self-Hosted); AI agents and assistants can query benchmark data and submit runs with the same permissions as a project API key (`bencher_run_*`), without needing the `bencher` CLI installed
...
- Add the project name and link as the first row of the PR comment report table (Thank you [@OmarTawfik](https://github.com/OmarTawfik))
- Create the GitHub Check with an `in_progress` status before benchmarks run and complete it with the results once they finish, so reruns no longer show a stale conclusion; if `bencher run` exits with an error before the results are posted, the check is marked as failed (Thank you [@OmarTawfik](https://github.com/OmarTawfik))
...
- `bencher run --github-actions` now always creates a `Bencher Report` GitHub Check (failing on an active Alert), so it can be used as a required status check in branch protection; the check is created on a best effort basis, so check creation failures are now warnings instead of errors (Thank you [@OmarTawfik](https://github.com/OmarTawfik))
...
- Fix URL fragment placed before query string per RFC 3986 (Thank you [@ricardoboss](https://github.com/ricardoboss))
- Fix `--start-point-hash` failure on non-existent hash by falling back to the start point branch's latest version (Thank you [@teofr](https://github.com/teofr))
...
- Add `[MemoryDiagnoser]` support to [the C# DotNet Adapter](https://bencher.dev/docs/explanation/adapters/#%EF%B8%8F%E2%83%A3-c-dotnet) (Thank you [@ricardoboss](https://github.com/ricardoboss))
- Add Dart `benchmark_harness` adapter (`dart_benchmark_harness`) (Thank you [@Louisvranderick](https://github.com/Louisvranderick))

## `v0.5.10`
- Fix GitHub PR comment exceeding maximum length (Thank you [@alexeyr-ci2](https://github.com/alexeyr-ci2))
- Improve GitHub API error messages
- Improve developer onboarding docs (Thank you [@Xenira](https://github.com/Xenira))
- Update GitHub Actions docs (Thank you [@pgoslatara](https://github.com/pgoslatara))

## `v0.5.9`
- Add Single Sign-On (SSO) API (Bencher Plus Enterprise feature)

## `v0.5.8`
- Allow the BenchmarkDotNet `namespace` to be optional (Thank you [@ntransw](https://github.com/ntransw))
- Fix Bencher Console for non-Bencher Plus builds (Thank you [@flokli](https://github.com/flokli))

## `v0.5.7`
- Fix workspace level feature unification for non-Bencher Plus builds (Thank you [@flokli](https://github.com/flokli))

## `v0.5.6`
- Fix Iai-Callgrind adapter parsing bug (Thank you [@gamma0987](https://github.com/gamma0987))
...
- **BREAKING CHANGE** Creating an on-the-fly project when the `CI` environment variable is set to `true` will now require the `--ci-on-the-fly` flag (Thank you [@jaqx0r](https://github.com/jaqx0r))
...
- Add support for GitHub Enterprise Server to `bencher` CLI (Thank you [@btoic](https://github.com/btoic))
...
- Add `--timeout` option to `bencher` CLI to set the timeout for all API requests
- Add `smtp.insecure_host` option to Bencher Self-Hosted API server configuration to allow insecure connections to SMTP servers (Thank you [@btoic](https://github.com/btoic))
...
- Use exponential scaling for Perf Plot y-axis when there is a > 10x relative difference (Thank you [@brianheineman](https://github.com/brianheineman))
- Improve new contributor onboarding experience (Thank you [@brianheineman](https://github.com/brianheineman))

## `v0.4.34`
- Add `--head` option to `bencher branch view` CLI subcommand
- Allow selection of up to two Measures in Perf Images
- Fix the Perf Plot Pinned tab
- Add `--build-time` and `--file-size` to PR comment ID
- Omit Threshold warning from PR comment when `--ci-only-thresholds` is used
- Fix flashing of "No data found" in Perf Plot
...
## `v0.4.26`
- Fix GitHub Action summary bug (Thank you [@dklassic](https://github.com/dklassic))
...
## `v0.4.24`
- Add GitHub Action Check when not running as a pull request (Thank you [@sagudev](https://github.com/sagudev), [@dklassic](https://github.com/dklassic), and [@wusyong](https://github.com/wusyong))

## `v0.4.23`
- Fix Console Alert Perf Plot button bug
- Fix benchmark.js adapter (`js_benchmark`) float parsing bug (Thank you [@arv](https://github.com/arv))
...
- Fix broken default version tag for `bencher up` (Thank you [@mwasplund](https://github.com/mwasplund))
...
- Track global bus events for `iai-callgrind` adapter (Thank you [@OmarTawfik](https://github.com/OmarTawfik))
...
- Parse results for multiple tools for `iai-callgrind` adapter (Thank you [@OmarTawfik](https://github.com/OmarTawfik))
...
- Add `--api-env`/`--console-env` and `--api-volume`/`--console-volume` to `bencher up` CLI command (Thank you [ricardaxel](https://github.com/ricardaxel))
...
- Fix `Add`, `Edit`, `Delete` button permissions bug (Thank you [eddyashton](https://github.com/eddyashton) for reporting this bug)
...
- Fix parsing new libtest bench Metrics as floats instead of integers ([#390](https://github.com/rust-lang/rust/pull/124774))
...
- Add an API endpoint for threshold statistics and a CLI `statistic` subcommand
...
- Wrote a [branch management](/docs/explanation/branches/) explanation page for using `bencher run`
