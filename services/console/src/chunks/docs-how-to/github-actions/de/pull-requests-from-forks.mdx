...
Aus Sicherheitsgründen sind Geheimnisse wie Ihr `BENCHER_API_KEY` und der `GITHUB_TOKEN` in GitHub Actions für Fork-PRs nicht verfügbar.
...
Keine Geheimnisse wie Ihr `BENCHER_API_KEY` und der `GITHUB_TOKEN` sind dort verfügbar.
...
wo Geheimnisse wie Ihr `BENCHER_API_KEY` und der `GITHUB_TOKEN` verfügbar sind.
