...
use bencher_json::{
    organization::JsonOrganizationPermission, JsonAllowed, JsonNewOrganization, JsonOrganization,
    ResourceId,
};
...
async fn get_allowed_inner(
    ...
    Ok(JsonAllowed {
        allowed: QueryOrganization::is_allowed_resource_id(
            ...
            &path_params.organization,
            auth_user,
            crate::model::user::organization::Permission::from(path_params.permission).into(),
        )
        .is_ok(),
    })
}
