...
- If `branch` does exist and either the `start-point` or the specified hash are different,
  a new instance of `branch` will be created from `start-point` at the specified hash.
...
1. Remove the old instance of `feature_branch`
2. Create a new instance of `feature_branch`
3. Copy of all the most recent data from `main` at hash `68b12758509ca5e2b1e3983525625e532eb5f562` to the new instance of `feature_branch`
4. `bencher run` would then proceed using the new instance of `feature_branch`
