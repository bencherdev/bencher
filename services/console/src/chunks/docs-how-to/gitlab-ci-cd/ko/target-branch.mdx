...
8. `--key` 옵션을 마스킹된 `BENCHER_API_KEY` 환경 변수에 설정합니다.
   자세한 내용은 [`--key` 문서][key option]를 참조하십시오.
...
11. `main` 브랜치, `debian:bullseye` 테스트베드, 및 `latency` 측정을 위한 임계값 설정:
    ...
    (예: `--thresholds-reset`)
12. 경고가 생성될 경우 명령이 실패하도록 `--error-on-alert` 플래그를 설정합니다.
    전체 개요는 [`--error-on-alert` 문서][alert err]를 참조하십시오.
    (예: `--error-on-alert`)
