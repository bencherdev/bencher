...
> 🐰 フィーチャーブランチで作業する際は、ベースである[Start Point][start point] Branchから既存のThresholdsをコピーしたい場合があります。これは、[`--start-point-clone-thresholds` flag][start point clone thresholds flag]を使用して可能です。ただし、`--thresholds-reset`フラグは、明示的に指定されていないすべてのクローンされたThresholdsも削除します。
