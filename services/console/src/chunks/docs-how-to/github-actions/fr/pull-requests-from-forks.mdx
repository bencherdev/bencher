...
Voici la manière sûre et suggérée pour ajouter [le benchmarking continu][continuous benchmarking] aux demandes de tirage de fourches. Cela nécessite deux workflows distincts. Le premier workflow exécute et met en cache les résultats du benchmark dans le contexte `pull_request`. Aucun secret tel que votre `BENCHER_API_KEY` et le `GITHUB_TOKEN` n'y est disponible. Ensuite, un second workflow télécharge les résultats des benchmarks mis en cache dans le contexte `workflow_run` et les télécharge vers Bencher. Cela fonctionne car `workflow_run` s'exécute dans le contexte de la branche par défaut du dépôt, où des secrets tels que votre `BENCHER_API_KEY` et le `GITHUB_TOKEN` sont disponibles. Le numéro de la demande de tirage, la branche principale et la branche de base utilisées dans le workflow initial `pull_request` doivent également être explicitement passés dans le workflow `workflow_run` car ils ne sont pas disponibles là-bas. Ces workflows ne s'exécuteront que s'ils existent sur la branche _par défaut_. Voir [utilisation des données du workflow déclencheur][github actions trigger workflow] pour un aperçu complet.
