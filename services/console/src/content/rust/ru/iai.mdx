...
Все три поддерживаются [по Bencher](/docs/ru/explanation/adapters/). Зачем выбрать Iai? 
Iai использует подсчёт инструкций вместо времени стенного часа. 
Это делает его идеальным для [непрерывного бенчмаркинга](/docs/ru/explanation/continuous-benchmarking/), то есть для бенчмаркинга в CI. 
...
> 🐰 Iai не обновлялся [больше 3 лет](https://github.com/bheisler/iai/commits/main/). Поэтому вы можете расматривать [использование Iai-Callgrind](/docs/ru/explanation/adapters/#-rust-iai-callgrind).
