## Pending `v0.3.22`
- **BREAKING CHANGE**: Update all `POST` API endpoints with creation semantics to return a `201` status code
- **BREAKING CHANGE**: Update all `PUT` and `PATCH` API endpoints with immediate update semantics to return a `200` status code
- **BREAKING CHANGE**: Update all `DELETE` API endpoints to return a `204` status code and an empty body
