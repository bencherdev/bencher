...
在[快速开始][quick start]和[Docker 自托管][docker self-hosted]教程中到此为止，让我们为我们的`Save Walter White`项目添加相对[持续基准测试][continuous benchmarking]。
...
2. 将`--project`选项设置为项目标识符。有关详细信息，请参阅[`--project`文档][project option]。（例如：`--project save-walter-white-1234abcd`）
...
在 CI 中第一次运行此命令时，将创建`feature-branch`分支，因为它尚不存在。新创建的`feature-branch`将没有起始点、现有数据或阈值。在后续运行中，旧的`feature-branch` [Head][head] 将被替换，并创建新的`feature-branch` [Head][head]，不含起始点、现有数据或阈值。
