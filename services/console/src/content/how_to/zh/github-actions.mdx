...
3. [选择分支](http://localhost:3000/docs/zh/explanation/branch-selection)来使用：
        1. 如果当前分支数据已经存在，则使用当前分支数据。（例如：`--if-branch '${{ github.head_ref }}'`）
        2. 如果PR目标分支的数据和阈值已经存在，则创建一个克隆。（例如：`--else-if-branch '${{ github.base_ref }}'`）
...
5. 查看[bencher run](/docs/zh/explanation/bencher-run)文档，全面了解如何使用`--ci-*`标志配置拉取请求评论的所有方式。
