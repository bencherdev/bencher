...
- If `branch` does exist and no Start Point is specified,
  a new instance of `branch` will be created without any historical data.
...
- If `branch` does exist and a Start Point is specified,
  a new instance of `branch` will be created using the Start Point.
...
2. Create a new instance of `feature_branch` with no historical data
3. `bencher run` would then proceed using the new instance of `feature_branch`
