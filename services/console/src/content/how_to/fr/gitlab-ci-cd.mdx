...
1. Optionnel : Définissez le flag `--adapter` ou la variable d'environnement `BENCHER_ADAPTER` au nom de l'adaptateur souhaité. Si cela n'est pas défini, alors l'Adaptateur `magic` sera utilisé. Consultez [les adaptateurs de benchmark](/docs/fr/explanation/adapters/) pour un aperçu complet. (ex: `BENCHER_ADAPTER: json`)
...
1. [Suivez vos benchmarks](/docs/fr/how-to/track-benchmarks/) avec la sous-commande CLI <code><a href="/docs/fr/explanation/bencher-run/">bencher run</a></code> :
    ...
    1. Définir la commande pour échouer si une alerte est générée. Pour qu'une alerte soit générée, un [Seuil](/docs/fr/explanation/thresholds/) doit déjà exister. (ex: `--err`)
