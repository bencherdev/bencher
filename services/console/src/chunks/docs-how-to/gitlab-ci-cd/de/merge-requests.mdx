...
8. Setzen Sie die `--key` Option auf die maskierte Umgebungsvariable `BENCHER_API_KEY`.
   Siehe [die `--key` Dokumentation][key option] für mehr Details.
   (z.B.: `--key "$BENCHER_API_KEY"`)
...
12. Setzen Sie das `--error-on-alert` Flag, um den Befehl bei Auslösung eines Alarms zum Abbruch zu bringen.
