...
10. Setzen Sie die `--key` Option auf das **Repository**-Geheimnis `BENCHER_API_KEY`.
   Siehe [die `--key` Doku][key option] für weitere Details.
...
14. Setzen Sie die `--error-on-alert` Flag, damit der Befehl fehlschlägt, wenn ein Alert generiert wird.
    Siehe [die `--error-on-alert` Doku][alert err] für einen vollständigen Überblick.
    (z.B.: `--error-on-alert`)
