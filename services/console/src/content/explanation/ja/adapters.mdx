...
import RustIaiCallgrind from "../../../chunks/explanation/ja/adapter-rust-iai-callgrind.mdx";
...
ベンチマークコマンドにも（例：`bencher run --file results.json "bencher mock > results.json"`）、ベンチマークコマンドなしにも（例：`bencher mock > results.json && bencher run --file results.json`）対応します。
