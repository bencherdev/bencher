...
Bencher é um conjunto de ferramentas de [execução contínua de benchmark](/docs/pt/explanation/continuous-benchmarking).
...
Se você estiver interessado em auto-hospedagem, confira o [tutorial do Bencher Docker](/docs/pt/tutorial/docker).
...
> 🐰 Dica: Confira a [documentação do subcomando `bencher run` CLI](/docs/pt/explanation/bencher-run) para uma visão completa de tudo que `bencher run` pode fazer!
...
> 🐰 Dica: Confira a [documentação sobre Limiares & Alertas](/docs/pt/explanation/thresholds) para uma visão completa de como regressões de desempenho são detectadas!
