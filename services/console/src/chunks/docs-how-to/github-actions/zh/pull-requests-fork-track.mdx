...
6. 将[`GITHUB_TOKEN` 的权限][github token permissions] 设置为 `checks` 和 `pull-requests` 的 `write`。
...
14. 将 `--key` 选项设置为 `BENCHER_API_KEY` **仓库**密钥。
    有关更多细节，请参见[`--key` 文档][key option]。
...
19. 设置 `--error-on-alert` 标志以在生成警报时命令失败。
   有关完整概述，请参见[`--error-on-alert` 文档][alert err]。
   （例如：`--error-on-alert`）
20. 将 `--adapter` 选项设置为由<code><a href="/zh/docs/reference/bencher-metric-format/#bencher-mock">bencher mock</a></code>生成的[Bencher Metric Format JSON (`json`)][bmf]。
   ...
   （例如：`--adapter json`）
21. 将 `--github-actions` 选项设置为 GitHub API 认证令牌，以使用[GitHub Actions `GITHUB_TOKEN` 环境变量][github token] 将结果作为 GitHub 检查和评论发布在 Pull Request 上。
...
[hash option]: /zh/docs/explanation/branches/#--hash-hash
