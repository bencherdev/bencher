...
then you can use the `bencher run` CLI command with the optional `--file` argument to specify that file path
...
This can be specified in the `bencher run` CLI command with the optional `--average` argument.
