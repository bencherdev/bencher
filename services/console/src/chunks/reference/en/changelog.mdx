...
- **BREAKING CHANGE** Move all CLI commands using `--org` and `--project` over to positional arguments, except `bencher run` and `bencher threshold create`
- Add a `bencher report create` CLI subcommand
- `bencher run` now tries to auto-fill the git commit hash for the report
- **BREAKING CHANGE** Rename `statistic` to `model`
