...
15. Установите флаг `--error-on-alert`, чтобы команда завершалась неудачей, если генерируется предупреждение.
...
16. Установите опцию `--adapter` на [Bencher Metric Format JSON (`json`)][bmf], который генерируется <code><a href="/ru/docs/reference/bencher-metric-format/#bencher-mock">bencher mock</a></code>.
   ...
   (пример: `--adapter json`)
17. Установите опцию `--github-actions` на токен аутентификации API GitHub для публикации результатов как проверка GitHub и комментарий к Pull Request с использованием
