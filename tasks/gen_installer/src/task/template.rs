...
impl Template {
    #[expect(
        clippy::use_debug,
        reason = "debug-prints template context for diagnostics"
    )]
...
#[expect(
    dead_code,
    reason = "variants used only in serialization via Serialize impl"
)]
...
#[expect(dead_code, reason = "variants used only in serialization via ZipStyle")]
