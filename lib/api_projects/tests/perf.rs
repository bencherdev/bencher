...
/// Insert a parameter set under a benchmark and return its row id.
fn create_parameter(
    server: &TestServer,
    benchmark_id: i32,
    set: &ParameterSet,
...
/// Add one grid point to a benchmark inside the fixture's report: a parameter set,
/// its own report benchmark row, and one metric on the fixture's measure.
...
/// Create a second benchmark under the fixture's project, with no grid point of its
/// own beyond the empty parameter set every benchmark is born with.
...
/// Add a named scalar to a report benchmark's measure.
...
/// A `JsonPerfQuery` over the fixture's dimensions, with `benchmarks` and the
/// parameters filter spelled out.
fn fixture_query(
    data: &PerfTestData,
...
/// The `parameters` query value for filter blobs spelled exactly as given, without
/// canonicalizing them on the way out.
///
/// Each blob is percent encoded as a list element, so the comma that separates
/// elements is the only literal comma, and then percent encoded again as a query
/// string value. Those are the two decodes the request undoes on the way in.
...
/// GET a perf path and return the parsed response.
...
// A benchmark reports as many lines as it has grid points. Without a filter every
// grid point is one line, and each line carries the parameter set it plots.
#[tokio::test]
async fn perf_fans_out_one_line_per_grid_point() {
    ...
    // The empty parameter set the benchmark was born with comes first, because the
    // lines come out in grid point creation order.
    assert_eq!(
...
// The filter is an OR across its elements and an AND within one: a grid point is
// queried when any one element names only keys it pins, with the same values.
#[tokio::test]
async fn perf_parameters_filter_is_an_or_of_ands() {
    ...
    // A one key element matches every superset of itself, and the empty parameter
    // set is a superset of nothing but itself.
    assert_eq!(
        line_parameters(&perf),
        vec![
...
// A filter that matches no grid point of one benchmark returns no lines for that
// benchmark, and none for the others either way.
#[tokio::test]
...
// A number is its canonical value, not its spelling, so a filter blob that spells
// one differently still hits the same grid point.
#[tokio::test]
...
// An empty `parameters` value is no filter at all, the same as leaving it off.
#[tokio::test]
...
// Every named scalar a measure ingested is in the map, the three names the metric
// triple maps onto included, and the boundaries list sits on the one that a
// threshold gated.
#[tokio::test]
async fn perf_metrics_map_carries_every_named_scalar() {
    ...
    // Only the gated named scalar carries a boundaries list, and it carries the
    // threshold, the boundary, and the alert together.
    let value = point
        .metrics
...
// A project that only ever reported the metric triple gets back every field it got
// back before, unchanged, with the new ones added beside them.
#[tokio::test]
...
// A measure that names no `value` still measured something, so it is still a point
// on the line. What it has no form for is the deprecated metric triple, which is
// absent there rather than making the whole point disappear.
#[tokio::test]
async fn perf_point_without_a_value_name_keeps_its_line() {
    ...
    // A second report benchmark on the fixture's grid point, in its own iteration so
    // it does not collide, whose measure names only `p99`.
    let mut conn = server.db_conn();
    let report_benchmark_uuid = ReportBenchmarkUuid::new();
    diesel::insert_into(schema::report_benchmark::table)
        .values((
    ...
    // The point that named only `p99` carries it in the map and nothing in the
    // deprecated fields.
