...
8. Установите опцию `--key` на замаскированную переменную среды `BENCHER_API_KEY`.
   Просмотрите [документацию по `--key`][key option] для более подробной информации.
   (например, `--key "$BENCHER_API_KEY"`)
...
12. Установите флаг `--error-on-alert` для вызова ошибки команды, если будет сгенерировано предупреждение.
   Просмотрите [документацию по `--error-on-alert`][alert err] для полного обзора.
