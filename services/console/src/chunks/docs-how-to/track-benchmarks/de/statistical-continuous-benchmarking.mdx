...
Anknüpfend an die Tutorials [Schnellstart][quick start] und [Docker Selbstgehostet][docker self-hosted], fügen wir statistisches [kontinuierliches Benchmarking][continuous benchmarking] zu unserem `Save Walter White` Projekt hinzu.
...
2. Setzen Sie die `--project` Option auf den Projektslug.
   Siehe [die `--project` Dokumentation][project option] für mehr Details. (z.B.: `--project save-walter-white-1234abcd`)
