...
pub struct SyncEngine<C> {
    ...
    /// Scan-space bounds `(start, end)` of a chunk whose upload failed. The
    /// PUT may have persisted server-side despite the error (lost response),
    /// so the retry caps its scan at `end` to re-derive the IDENTICAL
    /// segment (an idempotent overwrite of the same key); an uncapped retry
    /// after new commits would mint a longer segment with the same start,
    /// and the orphaned shorter object would break restore's offset chain.
    /// Cleared once the segment ships, and on any epoch, generation, or
    /// divergence transition (the WAL bytes it named are no longer the
    /// resume point).
    ship_retry: Option<(u64, u64)>,
    backoff: Backoff,
    ...
    /// tick). Also paces the stalled-drain escalation
    /// ([`Self::escalate_stalled_drain`]).
    oversized_retrigger_until: Option<i64>,
    /// Stalled oversized-drain checkpoint evaluations since construction
    /// (mirrors the `replica.oversized.stalled` counter; tests observe the
    /// stall through [`Self::oversized_stalled_count`]).
    oversized_stalled: u64,
}
...
impl<C> SyncEngine<C> {
    ...
    pub async fn checkpoint_once(&mut self) -> Result<CheckpointOutcome, SyncError> {
        ...
        if matches!(outcome, CheckpointOutcome::SkippedUnshipped)
    ...
    ///
    /// On the deadline (an incomplete drain), a storage failure, or a
    /// VACUOUS drain (a divergence pending its re-snapshot leaves no bound
    /// position, so nothing ships and the drain reports done) NO
    /// checkpoint runs and the remaining frames stay in the local WAL, but
    /// the meta's shipped-through-checkpoint proof (if any) is revoked
    /// first: the close-time checkpoint at process exit may backfill those
    /// frames and delete the WAL, and a surviving proof would let the next
    /// boot resume the lineage with them silently buried. If the WAL
    /// survives to the next boot, resume ships the tail by salt match; if
    /// it does not, the revoked proof forces a fresh generation whose
    /// snapshot recaptures the frames (churn, never loss).
    pub async fn final_sync(&mut self, deadline: Duration) -> Result<(), SyncError> {
        ...
        let drained = tokio::time::timeout(deadline, async {
            loop {
                match self.ship_once().await {
                    ...
                    Err(error) => {
                        slog::warn!(self.log,
                            "Final sync ship failed; remaining WAL frames stay local (the next boot may re-snapshot)";
                            "error" => %error);
                        return false;
                    },
                }
        ...
        .await;
        match drained {
            // A drain can also "complete" VACUOUSLY: after a divergence
            // pending its re-snapshot (position and awaited epoch both
            // cleared), `ship_pass` ships nothing and reports done while
            // committed unshipped frames may still sit in the WAL. The
            // shutdown checkpoint below would skip (nothing is provably
            // shipped), so a surviving meta proof would let the close-time
            // checkpoint bury those frames exactly like an incomplete
            // drain: revoke it instead of trusting the vacuous completion.
            Ok(true) if self.position.is_none() && self.awaiting.is_none() => {
                slog::warn!(
                    self.log,
                    "Final sync drained vacuously (divergence pending a re-snapshot); remaining WAL frames stay local (the next boot re-snapshots)"
                );
                self.revoke_meta_checkpoint_proof().await;
            },
            Ok(true) => {
            ...
            Ok(false) => self.revoke_meta_checkpoint_proof().await,
            Err(_elapsed) => {
                slog::warn!(
                    self.log,
                    "Final sync deadline reached; remaining WAL frames stay local (the next boot may re-snapshot)"
                );
                self.revoke_meta_checkpoint_proof().await;
            },
        }
    ...
    /// After an INCOMPLETE shutdown drain, unshipped committed frames remain
    /// in the local WAL, and the close-time checkpoint when the last
    /// in-process connection closes (which `wal_autocheckpoint = 0` does NOT
    /// disable) may backfill them and delete the WAL. A meta still proving
    /// "epoch shipped through checkpoint" would then let the next boot's
    /// WAL-less resume continue the lineage with those frames permanently
    /// absent from the replica (later epochs replay onto a base missing
    /// them: a torn restore). Revoke the proof so that boot resolves to a
    /// fresh generation instead; a boot that still finds the WAL resumes by
    /// salt match and ships the tail, proof or no proof. Best-effort: a
    /// failed revoke leaves the burial window open, and there is no better
    /// recourse inside the shutdown path than logging it.
    async fn revoke_meta_checkpoint_proof(&mut self) {
        self.epoch_checkpointed = false;
        let db_path = self.db.db_path.clone();
        let result = spawn_blocking(move || match ReplicaMeta::load(&db_path) {
            Ok(Some(meta)) if meta.epoch_shipped_through_checkpoint => ReplicaMeta {
        ...
        .await
        .map_err(SyncError::Join);
        if let Err(error) = result.and_then(|stored| stored.map_err(SyncError::Meta)) {
            slog::error!(self.log,
                "Failed to revoke the meta checkpoint proof after an incomplete shutdown drain; a close-time checkpoint could bury the unshipped WAL frames";
                "error" => %error);
        }
    }
    ...
    /// Stalled oversized-drain checkpoint evaluations since construction
    /// (mirrors the `replica.oversized.stalled` counter).
    #[must_use]
    pub fn oversized_stalled_count(&self) -> u64 {
        self.oversized_stalled
    }
    ...
    async fn ship_up_to_extent(
        ...
        while shipped < MAX_SEGMENTS_PER_PASS {
            ...
            let mut max_bytes = SEGMENT_MAX_BYTES.min(extent.saturating_sub(offset));
            match self.ship_retry {
                // Retry of a failed upload: cap the scan at the failed
                // chunk's end (a commit boundary, where the scanner stops)
                // so the re-scan reproduces exactly that chunk and key even
                // when new commits extended the extent since the failure.
                Some((start, end)) if start == offset => {
                    max_bytes = max_bytes.min(end.saturating_sub(offset));
                },
                // A remembered chunk for any other offset is stale.
                Some(_) => self.ship_retry = None,
                None => {},
            }
            let chunk = {
    ...
    async fn ship_chunk(
        ...
        let compressed = spawn_blocking(move || compress_segment(&raw))
            .await
            .map_err(SyncError::Join)??;
        if let Err(error) = self.storage.put(&key, Bytes::from(compressed)).await {
            // The PUT may have persisted despite the error: remember the
            // chunk's scan bounds so the retry re-derives this exact segment
            // instead of a longer duplicate-start sibling (see `ship_retry`).
            self.ship_retry = Some((start_offset, end_offset));
    ...
    fn transition_epoch(&mut self, position: &mut Position, header: &WalHeader) -> bool {
        // Whatever happens next (rebind, next epoch, or divergence), the WAL
        // restarted: a remembered failed-upload chunk names bytes that are no
        // longer the resume point.
        self.ship_retry = None;
        if self.oversized_drain == OversizedDrain::AwaitingRestart {
    ...
    /// The oversized drain is stalled: epoch 0 is pinned at the boundary (the
    /// ship path ships nothing), yet the drain checkpoint keeps deferring
    /// because a commit landed past the boundary after it was read. Those
    /// frames can never ship into epoch 0 (its first segment must start at
    /// offset 0), and backfilling them unshipped would tear every restore of
    /// the generation, so the checkpoint gate is load-bearing and stays.
    /// Instead the wedge is made loud (a dedicated counter plus an error log
    /// per stalled evaluation) and self-healing: once the
    /// [`OVERSIZED_RETRIGGER_MIN_SECS`] window expires, a FRESH oversized
    /// re-snapshot is minted, whose boundary is re-read past the wedging
    /// frames. Each attempt converges if no commit lands between its boundary
    /// read and its drain checkpoint, so a bursty writer converges on the
    /// first quiet moment instead of waiting for the snapshot interval.
    fn escalate_stalled_drain(&mut self) {
        self.oversized_stalled = self.oversized_stalled.saturating_add(1);
        #[cfg(feature = "otel")]
        bencher_otel::ApiMeter::increment(bencher_otel::ApiCounter::ReplicaOversizedStalled);
        let now = self.now_secs();
        let rate_limited = self
            .oversized_retrigger_until
            .is_some_and(|until| now < until);
        slog::error!(self.log,
            "Oversized-transaction drain is stalled: commits keep landing past the pinned boundary, so WAL shipping is paused and the local WAL grows without bound; quiesce writes briefly to let the drain complete, or wait for the automatic re-snapshot on the retrigger cadence";
            "escalating_now" => !rate_limited);
        if rate_limited {
            return;
        }
        self.position = None;
    ...
    async fn snapshot_finalize(&mut self, finalize: Box<FinalizeJob>) -> Result<(), SyncError> {
        ...
        self.awaiting = None;
        self.epoch_checkpointed = false;
        // A failed-upload chunk remembered from the OLD lineage must not cap
        // the new generation's first scan.
        self.ship_retry = None;
        ...
        // never saw. The committed snapshot stays, but only its BODY is
        // consistent (a single-step online backup): a restore that replays
        // epoch-0 segments on top can be a torn mixture missing the buried
        // frames, and it remains the restorable tip until the follow-up
        // generation scheduled here completes and recaptures them. The
        // recheck runs after the marker PUT because an external
        // checkpointer mid-snapshot is not proof of a tear, and aborting
        // here would discard a whole uploaded body on a heuristic (see
        // `snapshot_survives_external_checkpoint_mid_copy`, which pins the
        // commit-then-follow-up behavior). Shadow mode skips this
        // (Litestream checkpoints constantly; the daily verification is
        // the backstop there).
        if !self.shadow {
            let db_path = self.db.db_path.clone();
