...
| ----------- | -------------------------------------------------- | ----------------------------------------------------------------------- |
| `ready`     | Der Runner ist im Leerlauf und fordert einen Job an | Optionales `poll_timeout` (1-900s) und `runner`-Metadaten (`os`, `arch`, `version`, optionaler Update-`channel` und optionale Binary-`checksum`) |
