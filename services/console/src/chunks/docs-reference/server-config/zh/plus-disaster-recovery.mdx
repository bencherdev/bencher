...
- `checkpoint_interval`: （可选）Litestream执行非阻塞PASSIVE检查点的频率，不受页面数量影响。如果有活跃的读取器或写入器，Litestream将跳过检查点。默认值为`1m`。
- `min_checkpoint_page_count`: （可选）触发非阻塞PASSIVE检查点前的最小WAL页面数（每页约4KB）。如果有活跃的读取器或写入器，Litestream将跳过检查点。默认值为`1000`（约4MB）。
- `truncate_page_n`: （可选）阻塞TRUNCATE检查点的页面数阈值。当达到此阈值时，Litestream会执行阻塞检查点，等待所有读取器和写入器完成，这可能在负载下导致`database is locked`错误。**默认值为`0`（禁用）。** 仅在需要WAL文件大小上限并了解阻塞影响的情况下才设置为非零值。
