...
6. Setzen Sie [die Berechtigungen für den `GITHUB_TOKEN`][github token permissions]
   auf `write` für `checks` und `pull-requests`.
...
14. Setzen Sie die `--key` Option auf das **Repository**-Geheimnis `BENCHER_API_KEY`.
...
19. Setzen Sie die `--error-on-alert` Flagge, um den Befehl fehlschlagen zu lassen, wenn ein Alarm erzeugt wird.
...
20. Setzen Sie die `--adapter` Option auf [Bencher Metric Format JSON (`json`)][bmf], das von <code><a href="/de/docs/reference/bencher-metric-format/#bencher-mock">bencher mock</a></code> erzeugt wird.
   ...
   (z.B.: `--adapter json`)
21. Setzen Sie die `--github-actions` Option auf das GitHub API-Authentifizierungstoken, um Ergebnisse als GitHub-Prüfung und als Kommentar im Pull Request zu posten, unter Verwendung
...
[hash option]: /de/docs/explanation/branches/#--hash-hash
