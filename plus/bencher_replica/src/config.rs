...
pub struct ReplicaConfig {
    ...
    /// (tests lower it to exercise the poison path). A committed transaction
    /// beyond the bound degrades replication to a re-snapshot that only
    /// converges in a quiet write window (see the crate-level operational
    /// assumptions); avoid `VACUUM` on a replicated database near the bound.
