...
8. Configura la opción `--key` a la variable de entorno enmascarada `BENCHER_API_KEY`.
   Consulta la documentación de [la opción `--key`][key option] para más detalles.
...
12. Activa el indicador `--error-on-alert` para fallar el comando si se genera una Alerta.
   Consulta la documentación de [la opción `--error-on-alert`][alert err] para una visión completa.
   (ej: `--error-on-alert`)
