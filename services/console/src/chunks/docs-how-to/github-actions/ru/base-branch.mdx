...
10. Установите опцию `--key` на секрет **Repositorий** `BENCHER_API_KEY`.
   См. [документацию о `--key`][key option] для получения более подробной информации.
...
14. Установите флаг `--error-on-alert`, чтобы команда завершалась с ошибкой, если будет сформировано оповещение.
    См. [документацию о `--error-on-alert`][alert err] для получения полной информации.
