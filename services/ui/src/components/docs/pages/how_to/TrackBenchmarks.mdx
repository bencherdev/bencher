The easiest way to track your benchmarks is the `bencher run` CLI command.
See the [benchmarking overview](/docs/explanation/benchmarking) for a more in-depth explanation.
This is an example of a `bencher run` CLI command to track benchmarks on a feature branch aptly named `feature-branch`:
...
1. There are several options for setting the project branch. See [branch selection](/docs/how-to/branch-selection) for a full overview.
...
1. Optional: Set the `--testbed` flag or the `BENCHER_TESTBED` environment variable to the Testbed slug or UUID. If this is not set, then the `localhost` Testbed will be used. (ex: `--testbed phoenix`)
...
This is an example of a `bencher run` CLI command to perform relative benchmarking on a feature branch aptly named `feature-branch`:
