...
This should be the URL for your Bencher UI host. (ex `"https://bencher.example.com"`)
For Bencher Cloud this value is `"https://bencher.dev"`.
The default value for `endpoint` is `"http://localhost:3000"`.

## `secret_key`

This is the key used to generate all tokens. **IT SHOULD BE VERY SECURE!**
The default value for `secret_key` is a randomly generated secret key.
Whenever `secret_key` is logged, it will appear obfuscated as `************`.
...
| data_store.secret_access_key | "AA3Chr-JSF5sUQqKwayx-FvCfZKsMev-5BqPpcFC3m7"                                 | ---               | Only if data_store.service = "aws_s3" | If data_store.service = "aws_s3", this property specifies the AWS secret access key. See also data_store.service.                                                                                |
...
| secret     | "WM3F2u9cqSNdBPLfy9sJ5kk9" | ---     | Yes      | Specifies the secret for the username at the SMTP host.                 |
