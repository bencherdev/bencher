...
- `play_game`: Takes in an unsigned integer `i`, calls `fizz_buzz` with that number, and prints the result.
- `fizz_buzz`: Takes an unsigned integer `i` and performs the actual `Fizz`, `Buzz`, `FizzBuzz`, or number logic returning the result as a string.
...
- Use the `Bencher` instance to benchmark `play_game` for the numbers `1` to `100` inclusive by running it several times.
