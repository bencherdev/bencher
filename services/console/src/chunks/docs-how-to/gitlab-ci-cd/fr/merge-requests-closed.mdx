...
9. Définissez l'option `--key` sur la variable d'environnement masquée `BENCHER_API_KEY`.
   Consultez la [documentation de l'option `--key`][key option] pour plus de détails.
