...
[Inicio Rápido][quickstart] y [Autohospedado con Docker][docker self-hosted],
vamos a agregar [Benchmarking Continuo][continuous benchmarking] Relativo a nuestro proyecto `Salvar a Walter White`.

> 🐰 Asegúrate de haber
> [creado un token de API y configurado como la variable de entorno `BENCHER_API_KEY`][create an api token]
> antes de continuar.
