...
5. `GITHUB_TOKEN`의 [권한을 설정][github token permissions]하여 `checks` 및 `pull-requests`에 `write`를 부여합니다.
...
15. 알림이 생성되면 명령어를 실패시키도록 `--error-on-alert` 플래그를 설정합니다.
...
16. <code><a href="/ko/docs/reference/bencher-metric-format/#bencher-mock">bencher mock</a></code>에 의해 생성된 [Bencher Metric Format JSON (`json`)][bmf]에 `--adapter` 옵션을 설정합니다.
   ...
   (예: `--adapter json`)
17. [GitHub Actions `GITHUB_TOKEN` 환경 변수][github token]을 사용하여 결과를 GitHub Check 및 Pull Request 댓글로 게시하는 [GitHub API 인증 토큰][github actions option]에 `--github-actions` 옵션을 설정합니다.
...
[start point hash]: /ko/docs/explanation/branches/#--start-point-hash-hash
[start point clone thresholds]: /ko/docs/explanation/branches/#--start-point-clone-thresholds
[start point reset]: /ko/docs/explanation/branches/#--start-point-reset
