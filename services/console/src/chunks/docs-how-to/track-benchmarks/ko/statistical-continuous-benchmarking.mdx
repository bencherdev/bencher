...
[빠른 시작][quickstart] 및 [Docker 자체 호스팅][docker self-hosted] 튜토리얼에서 중단했던 곳에서 시작하여, `claimed` 프로젝트에 통계적인 [지속적 벤치마킹][continuous benchmarking]을 추가하겠습니다.

> 🐰 계속하기 전에
> [API 토큰을 생성하고 `BENCHER_API_KEY` 환경 변수로 설정했는지][create an api token]
> 확인하세요!
