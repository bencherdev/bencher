// https://github.com/astral-sh/uv/blob/591f38c25e577d29bb4fd0dde7cdb614f3129bfc/crates/uv-configuration/src/trusted_host.rs
...
impl FromStr for TrustedHost {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let (scheme, s) = if let Some(s) = s.strip_prefix("https://") {
            (Some("https".to_owned()), s)
        } else if let Some(s) = s.strip_prefix("http://") {
            (Some("http".to_owned()), s)
        } else {
            (None, s)
        };
        ...
        let host = parts
            ...
            .ok_or_else(|| TrustedHostError::MissingHost(s.to_owned()))?;
        ...
        let port = parts
            ...
            .map(str::parse)
            .transpose()
            .map_err(|_e| TrustedHostError::InvalidPort(s.to_owned()))?;
    ...
    fn json_schema(_gen: &mut schemars::gen::SchemaGenerator) -> schemars::schema::Schema {
        schemars::schema::SchemaObject {
            ...
            metadata: Some(Box::new(schemars::schema::Metadata {
                description: Some("A host or host-port pair.".to_owned()),
