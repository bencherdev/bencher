...
pub fn metrics_data(
    ...
    let max_sample_size = model.max_sample_size.map(i64::from);

    let sql = history_sql(
        detector.spec_id.is_some(),
        start_time.is_some(),
        max_sample_size.is_some(),
    );
    let mut query = diesel::sql_query(sql)
        .into_boxed::<Sqlite>()
        .bind::<Integer, _>(detector.head_id)
        .bind::<Integer, _>(detector.testbed_id)
        .bind::<Integer, _>(benchmark_id)
        .bind::<Integer, _>(parameter_id)
        .bind::<Integer, _>(detector.measure_id)
        .bind::<Text, _>(MetricName::value());
    if let Some(spec_id) = detector.spec_id {
        query = query.bind::<Integer, _>(spec_id);
    }
    if let Some(start_time) = start_time {
        query = query.bind::<BigInt, _>(start_time);
    }
    if let Some(max_sample_size) = max_sample_size {
        query = query.bind::<BigInt, _>(max_sample_size);
    }

    let data = query
        .load::<HistoryValue>(conn)
...
fn history_sql(spec: bool, window: bool, limit: bool) -> String {
    let mut sql = String::from(
        "SELECT metric.value FROM head_version \
         CROSS JOIN report \
         CROSS JOIN report_benchmark INDEXED BY index_report_benchmark_benchmark_report \
         CROSS JOIN metric \
         WHERE head_version.head_id = ? AND report.testbed_id = ? \
         AND report_benchmark.benchmark_id = ? AND report_benchmark.parameter_id = ? \
         AND metric.measure_id = ? AND metric.name = ? \
    ...
    if spec {
        sql.push_str(" AND report.spec_id = ?");
    }
    if window {
        sql.push_str(" AND report.start_time >= ?");
    }
    sql.push_str(
...
mod tests {
    ...
    use super::{history_sql, metrics_data};
    use crate::{
        model::{
            project::{
                ProjectId,
    ...
    fn assert_newest_first_plan(conn: &mut SqliteConnection) {
        for shape in 0..8 {
            let sql = history_sql(shape & 1 != 0, shape & 2 != 0, shape & 4 != 0);
            let plan: Vec<String> = diesel::sql_query(format!("EXPLAIN QUERY PLAN {sql}"))
                .load::<PlanStep>(conn)
                .expect("Failed to explain the history query")
                .into_iter()
                .map(|step| step.detail)
                .collect();
            assert!(
                plan.iter().any(|detail| detail
                    .contains("head_version USING COVERING INDEX sqlite_autoindex_head_version_1")),
                "{plan:?}"
            );
            assert!(
                !plan
                    .iter()
                    .any(|detail| detail.contains("B-TREE FOR ORDER BY")),
                "{plan:?}"
            );
        }
    }
    ...
    #[test]
    fn metrics_data_plan_walks_head_newest_first() {
        let mut fixture = fixture();
        assert_newest_first_plan(&mut fixture.conn);
