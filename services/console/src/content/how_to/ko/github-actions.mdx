...
5. API 토큰은 반드시 존재해야 합니다.  `BENCHER_API_TOKEN`을 **Repository** 비밀로 추가합니다. (예: `Repo -> Settings -> Secrets and variables -> Actions -> New repository secret`) `--token` 플래그를 설정하거나 `BENCHER_API_TOKEN` 환경 변수를 API 토큰으로 설정합니다. (예: `BENCHER_API_TOKEN: ${{ secrets.BENCHER_API_TOKEN }}`)
...
10. <code><a href="/docs/ko/explanation/bencher-run">bencher run</a></code> CLI 하위 명령을 사용하여 [벤치마크를 추적](/docs/ko/how-to/track-benchmarks)합니다:
    ...
    2.  Alert이 생성될 경우 명령이 실패하도록 설정합니다. (예: `--err`) Alert이 생성되려면 [Threshold](/docs/ko/explanation/thresholds)가 이미 존재해야 합니다.
...
1. GitHub Actions `workflow` 파일을 생성합니다. (예: `.github/workflows/benchmark.yml`)
1. `main` 브랜치로 `push` 이벤트와 `pull_request` 이벤트를 실행합니다. PR 브랜치로의 push가 두 번 실행되는 것을 방지하기 위해 `push`를 선택한 브랜치(예: `main`)에만 **제한**하는 것이 중요합니다!
2. 항상 `main` 브랜치를 사용하는 대신, [GitHub Action 기본 환경 변수](https://docs.github.com/ko/actions/learn-github-actions/variables#default-environment-variables)를 사용하여 보다 세밀하게 컨텐츠를 가져옵니다:
        1. 기존 브랜치 데이터가 있으면 사용합니다. (예: `--if-branch '${{ github.head_ref }}'`)
        2. 이미 존재하는 PR 타겟 브랜치 데이터와 임계값의 클론을 생성합니다. (예: `--else-if-branch '${{ github.base_ref }}'`)
        3. 그렇지 않으면, `main` 브랜치 데이터와 임계 값의 클론을 생성합니다. (예: `--else-if-branch main`)
        4. 프로젝트 브랜치를 설정하는 데에는 여러 옵션이 있습니다. 전체 개요는 [브랜치 선택](/docs/ko/explanation/branch-selection)을 참조하세요.
3. GitHub API 인증 토큰을 설정합니다. (예: `--github-actions "${{ secrets.GITHUB_TOKEN }}"`) 이 옵션이 풀 요청의 일부로 설정되면 결과는 풀 요청에 댓글로 추가됩니다. 이는 [GitHub Actions `GITHUB_TOKEN` 환경 변수](https://docs.github.com/ko/actions/security-guides/automatic-token-authentication)를 사용합니다.
4. pull request 코멘트를 `--ci-*` 플래그를 사용하여 모든 방법으로 구성하는 방법에 대한 전체 개요는 [bencher run](/docs/ko/explanation/bencher-run) 문서를 참조하세요.
...
1. GitHub Actions `workflow` 파일을 생성합니다. (예: `.github/workflows/pr_benchmark.yml`)
...
1. (보이지 않음) 두 번째 GitHub Actions `workflow` 파일을 만들고 위의 초기 예제를 사용하여 `main` 브랜치로 `push` 이벤트를 실행합니다. (예: `.github/workflows/benchmark.yml`)
...
1. `Run and Cache Benchmarks` 워크플로우 파일을 만듭니다.
...
1. 두 번째 워크플로우 파일, `Track Benchmarks`를 만듭니다.
...
1. (보이지 않음) 세 번째 GitHub Actions `workflow` 파일을 만들고 위의 초기 예를 사용하여 `main` 브랜치로 `push` 이벤트를 실행합니다. (예: `.github/workflows/benchmark.yml`)
