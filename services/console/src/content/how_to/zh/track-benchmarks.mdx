...
追踪你的基准最简单的方法就是 <code><a href="/docs/zh/explanation/bencher-run">bencher run</a></code> 命令行子命令。
对于更深入的解释，请查看 [基准测试概览](/docs/zh/explanation/benchmarking)。
下面是一个<code><a href="/docs/zh/explanation/bencher-run">bencher run</a></code>命令行子命令的示例，用于追踪在一个合适命名为`feature-branch`的功能分支上的基准:
...
1. 选择设置: 设置 `--adapter` 标志或 `BENCHER_ADAPTER` 环境变量为所需的适配器名称。如果没有设置则会使用 `magic` 适配器。查看 [基准测试马具适配器](/docs/zh/explanation/adapters) 获得全部概览。（例如： `--adapter json`）
1. 有多种方式可以设置项目分支。查看 [分支选择](/docs/zh/explanation/branch-selection) 获得全部概览。
...
1. 如果生成一个警告就让命令执行失败。为了警告被生成，一项 [阈值](/docs/zh/explanation/thresholds) 必须已经存在。（例如： `--err`）
...
下面是一个<code><a href="/docs/zh/explanation/bencher-run">bencher run</a></code>命令行子命令的示例，用于在一个合适命名为`feature-branch`的功能分支上执行相对基准:
...
1. 对目标分支运行 `bencher run`:
    ...
    1. 运行基准三次。（例如： `--iter 3`）
1. 检出功能分支。（例如： `feature-branch`）
1. 为功能分支创建一个 [阈值](/docs/zh/explanation/thresholds):
    1. 基准的度量为延迟。（例如： `--metric-kind latency`）
    ...
    1. 设置一个 95.0% 的右边界，因为更大的延迟表示表现出回退。（例如： `--right-side 0.95`）
1. 对功能分支运行 `bencher run`:
    1. 分支将存在，因为刚刚被创建。（例如： `--if-branch "$FEATURE_BRANCH"`）
    1. 运行测试三次。（例如： `--iter 3`）
    1. 将所有三个度量内折为最小值。（例如： `--fold min`）
    1. 如果 [阈值](/docs/zh/explanation/thresholds) 生成了警告则让命令执行失败。（例如： `--err`）
...
<h2><a href="/docs/zh/how-to/github-actions">添加 Bencher 到 GitHub Actions ➡</a></h2>
<h2><a href="/docs/zh/how-to/gitlab-ci-cd">添加 Bencher 到 GitLab CI/CD ➡</a></h2>