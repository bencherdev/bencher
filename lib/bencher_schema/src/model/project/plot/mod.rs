...
pub struct QueryPlot {
    ...
    pub y_axis: YAxis,
    pub window: Window,
    /// The variants this plot draws, when it does not draw every one.
...
pub struct InsertPlot {
    ...
    pub y_axis: YAxis,
    pub window: Window,
    /// Stored canonically: a filter that draws every variant is no filter at
...
pub struct UpdatePlot {
    ...
    /// variant, and `Some(Some(filter))` replaces it.
    pub parameters: Option<Option<ParameterFilter>>,
    pub modified: DateTime,
}

/// The canonical stored form of a filter: a filter that draws every variant is
...
/// filter puts the plot back on every variant.
#[expect(
