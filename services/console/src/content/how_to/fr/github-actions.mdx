...
1. Créez un fichier `workflow` GitHub Actions. (ex: `.github/workflows/benchmark.yml`)
...
5. Le token d'API doit déjà exister. Ajoutez `BENCHER_API_TOKEN` en tant que secret de **dépôt**. (ex: `Dépôt -> Settings -> Secrets and variables -> Actions -> New repository secret`) Définissez l'option `--token` ou la variable d'environnement `BENCHER_API_TOKEN` pour le token d'API. (ex: `BENCHER_API_TOKEN: ${{ secrets.BENCHER_API_TOKEN }}`)
...
7. Facultatif: Définissez l'option `--adapter` ou la variable d'environnement `BENCHER_ADAPTER` pour le nom de l'adaptateur désiré. (ex: `BENCHER_ADAPTER: json`) Si cela n'est pas défini, alors l'adaptateur `magic` sera utilisé. Voir [adaptateurs de banc de test](/docs/fr/explanation/adapters) pour un aperçu complet.
...
10. [Suivez vos benchmarks](/docs/fr/how-to/track-benchmarks) avec la sous-commande CLI <code><a href="/docs/fr/explanation/bencher-run">bencher run</a></code>:
    ...
    2. Configurez la commande pour échouer si une alerte est générée. (ex: `--err`) Pour qu'une alerte soit générée, un [Seuil](/docs/fr/explanation/thresholds) doit déjà exister.
...
1. Créez un fichier `workflow` GitHub Actions. (ex: `.github/workflows/benchmark.yml`)
1. Exécutez lors d'événements `push` vers la branche `main` et lors d'événements `pull_request`. Il est important de limiter l'exécution lors d'événements `push` **uniquement** aux branches sélectionnées (ex: `main`) pour éviter que les poussées vers les branches de demande de fusion s'exécutent deux fois !
2. Au lieu d'utiliser toujours la branche `main`, utilisez les [variables d'environnement par défaut de GitHub Action](https://docs.github.com/fr/actions/learn-github-actions/variables#default-environment-variables) pour :
        1. Utiliser les données de la branche en cours si elles existent déjà. (ex: `--if-branch '${{ github.head_ref }}'`)
        2. Créer un clone des données et des seuils de la branche cible de la demande de fusion si elles existent déjà. (ex: `--else-if-branch '${{ github.base_ref }}'`)
        3. Sinon, créer un clone des données et des seuils de la branche `main`. (ex: `--else-if-branch main`)
        4. Il existe plusieurs options pour définir la branche du projet. Voir [la sélection de branche](/docs/fr/explanation/branch-selection) pour un aperçu complet.
3. Définissez le token d'authentification API GitHub. (ex: `--github-actions "${{ secrets.GITHUB_TOKEN }}"`) Lorsque cette option est définie dans le cadre d'une demande de fusion, les résultats sont ajoutés à la demande de fusion sous forme de commentaire. Ceci utilise [la variable d'environnement `GITHUB_TOKEN` de GitHub Actions](https://docs.github.com/fr/actions/security-guides/automatic-token-authentication).
4. Consultez la documentation [bencher run](/docs/fr/explanation/bencher-run) pour un aperçu complet de toutes les manières de configurer le commentaire de la demande de fusion avec les options `--ci-*`.
...
1. Créez un fichier `workflow` GitHub Actions. (ex: `.github/workflows/pr_benchmark.yml`)
...
5. (Non montré) Créez un deuxième fichier `workflow` GitHub Actions et utilisez l'exemple initial ci-dessus pour exécuter lors d'événements `push` vers la branche `main`. (ex: `.github/workflows/benchmark.yml`)
...
1. Créez un fichier de workflow `Run and Cache Benchmarks`.
...
1. Créez un second fichier de workflow, `Track Benchmarks`.
...
7. (Non montré) Créez un troisième fichier de workflow GitHub Actions et utilisez l'exemple initial ci-dessus pour exécuter lors d'événements `push` vers la branche `main`. (ex: `.github/workflows/benchmark.yml`)
...
<h2><a href="/docs/fr/explanation/benchmarking">Continuer: Aperçu du Benchmarking ➡</a></h2>
