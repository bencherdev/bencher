...
同样**非常**重要的是，必须用**单**引号把 PR 分支名称（头引用）包裹起来。 (例如：`--if-branch '${{ github.head_ref }}'`)。否则，攻击者可以创建一个恶意命名的分支，进行命令注入。看一下这篇[GitHub安全实验室的文章](https://securitylab.github.com/research/github-actions-untrusted-input/)，其中对防止来自不受信任输入的 pwn 请求进行了全面的概述。
