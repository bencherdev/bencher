...
fügen wir Relatives [Continuous Benchmarking][continuous benchmarking] zu unserem `Save Walter White` Projekt hinzu.
...
Der neue `feature-branch` wird _nicht_ über einen Startpunkt, vorhandene Daten oder Thresholds verfügen.
Bei nachfolgenden Läufen wird der alte `feature-branch` [Head][head] ersetzt
und ein neuer `feature-branch` [Head][head] wird erstellt, ohne einen Startpunkt, vorhandene Daten oder Thresholds.
