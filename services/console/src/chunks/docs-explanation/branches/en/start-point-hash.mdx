...
- If `branch` does not exist and `start-point` does not exist at the specified hash,
  then the `branch` will be created from the latest `start-point` version.
  If `start-point` does not exist, then the `branch` will be created without any historical data.
- If `branch` does exist and `start-point` does not exist at the specified hash,
  a new `branch` [Head][head] will be created from the latest `start-point` version.
  If `start-point` does not exist, then this option is ignored.
