...
8. 隠された`BENCHER_API_KEY`環境変数に`--key`オプションを設定します。
   詳細については、[`--key` ドキュメント][key option]をご覧ください。
...
11. `main`ブランチ、`debian:bullseye`テストベッド、および`latency`測定基準のための閾値を設定します。
    ...
    (例: `--thresholds-reset`)
12. アラートが生成された場合にコマンドが失敗するように`--error-on-alert`フラグを設定します。
   詳細については、[`--error-on-alert` ドキュメント][alert err]をご覧ください。
   (例: `--error-on-alert`)
