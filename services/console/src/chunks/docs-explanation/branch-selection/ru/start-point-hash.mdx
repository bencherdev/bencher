...
- Если `branch` не существует и `start-point` не существует с указанным хэшем,
  тогда `branch` будет создан из последней версии `start-point`.
  Если `start-point` не существует, тогда `branch` будет создан без каких-либо исторических данных.
- Если `branch` существует и `start-point` не существует с указанным хэшем,
  будет создана новая [Head][head] ветка `branch` из последней версии `start-point`.
  Если `start-point` не существует, то эта опция игнорируется.
