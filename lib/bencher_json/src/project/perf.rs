...
pub mod table {
    ...
    /// The header of the column that names each line's variant.
    ///
    /// A project that never reported any parameters has nothing to tell its lines
    ...
    impl From<JsonPerf> for Table {
        fn from(json_perf: JsonPerf) -> Self {
            // One non-empty variant anywhere in the query is what makes the column worth
            // a column: without one, every line is the benchmark's only variant.
            let has_variants = json_perf
                .results
                .iter()
                .any(|result| !result.variant.parameters.is_empty());

            let mut perf_table = Vec::new();
            for result in json_perf.results {
                let parameters = result.variant.parameters.canonical();
    ...
    pub struct PerfTable {
        ...
        pub benchmark: JsonBenchmark,
        /// The canonical spelling of the variant this line plots.
        ///
        /// The column sits between the benchmark and the measure, the way the
        /// variant sits between them in a perf result, and it is removed entirely
        /// when no line of the query plots a non-empty variant.
    ...
    mod tests {
        ...
        // A query whose every line plots the empty variant is the query every
        // project made before a benchmark could have more than one variant. It
        // prints exactly the table it always printed, column for column.
        #[test]
        fn table_without_variants() {
            ...
            assert_eq!(
                ...
                "the table a project without variants prints"
            );
        }
