...
如果您计划接受来自 fork 的拉取请求，这通常在公共开源项目中发生，那么您需要稍微不同地处理事情。出于安全原因，像 `BENCHER_API_KEY` 和 `GITHUB_TOKEN` 这样的秘密在针对 fork 提交的 GitHub Actions 中不可用。也就是说，如果外部贡献者从 fork 中打开拉取请求，上述示例将 **不** 能工作。请参阅这个 [GitHub Security Lab 的文章][github actions pwn requests] 和 [这篇博客文章][nathandavison.com blog]，以了解完整概述如何防止恶意请求。
