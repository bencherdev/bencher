...
If there is a [Threshold](/docs/explanation/thresholds) set, then the new Metrics are compared against the historical Metrics for each Benchmark present in the Report. If a regression is detected, then an Alert will be generated.
...
so the renamed performance regression test will be considered a new Benchmark.
...
> 🐰 Let Us Know: Would like [a way to map an old Benchmark to a new Benchmark](https://github.com/bencherdev/bencher/issues/71)?
...
Reports are most often generated using the <code><a href="/docs/reference/bencher-run">bencher run</a></code> CLI subcommand.
...
When using the <code><a href="/docs/reference/bencher-run">bencher run</a></code> CLI subcommand,
