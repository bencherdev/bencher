...
3. [Selecione a branch](http://localhost:3000/docs/pt/explanation/branch-selection) para usar:
        ...
        2. Crie um clone dos dados e limites da branch alvo do PR, se existirem. (ex: `--else-if-branch '${{ github.base_ref }}'`)
...
5. Veja a documentação de [execução do bencher](/docs/pt/explanation/bencher-run) para uma visão geral completa de todas as maneiras de configurar o comentário do pull request com as flags `--ci-*`.
