...
See the [CLI API Client][cli api client] section for more on this point.
...
When I started developing Bencher, I tried to keep it as simple as possible.
...
In the future when I switch to Kamal, I'll also be moving off of Fly.io.
...
But with Litestream, you can't have more than one machine!
...
All of these capabilities really helps shrink the amount of code I have to maintain
for the `bencher` CLI.
...
When I picked [Dropshot][github dropshot] as Bencher's
[backend HTTP framework][backend http framework],
one of its biggest selling points was its built-in ability to generate an OpenAPI spec.
...
```
client.instance_create(org, proj, body)
```

or a builder pattern:

```
client.instance_create().organization_name("org").project_name("proj").body(body)
```

Personally, I prefer the latter,
so that's what Bencher uses.
Progenitor can also generate an entire clap CLI to interact with API.
...
When I started working on Bencher, folks were calling for [the end of localhost][dx tips].
...
[github bencher dev container]: https://github.com/bencherdev/bencher/tree/v0.4.33/.devcontainer
...
At the time, you could only use Dagger via this esoteric language called [CUE][github cue].
