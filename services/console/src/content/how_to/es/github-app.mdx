...
### Consola UI
Configura la variable de entorno `GITHUB_CLIENT_ID` cuando ejecutes la imagen Docker `bencher-ui`. (ej: `GITHUB_CLIENT_ID=Iv1.12864abcd1232048`) Si falta el ID del cliente, los botones de GitHub no aparecerán en la interfaz de usuario.

### Servidor API
Añade el `client_id` y el `client_secret` a [la sección `plus.github` de la configuración del servidor](/docs/es/reference/server-config/#plusgithub). Si falta la sección de GitHub, entonces el servidor fallará cualquier petición de autenticación de GitHub.
...
<h2><a href="/docs/es/how-to/github-actions/">Añadir Bencher a GitHub Actions ➡</a></h2>