...
e caso contrário marcado como `unknown` (o contato com o Runner foi perdido).
...
O limite conta a partir de quando o Job começou a rodar, ou de quando foi reivindicado se nunca começou, e também se aplica a um Job `unknown`.
...
caso contrário, o Job é marcado como `unknown`, ou `canceled` se tiver excedido o timeout rígido.
O servidor verifica um Job `unknown` novamente em seu prazo.
Na inicialização, o servidor também marca Jobs `claimed` órfãos como `unknown`,
reagenda timeouts para Jobs em andamento e `unknown`
e reprocessa Jobs `completed` cujos resultados foram armazenados mas ainda não analisados;
um Job `completed` cujos resultados foram perdidos é marcado como `unknown` para que o Runner possa reenviá-los.
...
O resultado de um Runner resolve um Job `unknown`.
Um Job `failed` ou `canceled` nunca muda de estado novamente: um resultado tardio é confirmado, para que o Runner pare de reenviá-lo, e descartado.
