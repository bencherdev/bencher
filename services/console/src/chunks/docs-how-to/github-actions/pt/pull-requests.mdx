...
15. Defina o flag `--error-on-alert` para falhar o comando se um Alerta for gerado.
...
16. Defina a opção `--adapter` para [Bencher Metric Format JSON (`json`)][bmf] que é gerado por <code><a href="/pt/docs/reference/bencher-metric-format/#bencher-mock">bencher mock</a></code>.
   ...
   (ex: `--adapter json`)
17. Defina a opção `--github-actions` para o token de autenticação da API do GitHub para postar resultados como uma Verificação do GitHub e um comentário no Pull Request usando
