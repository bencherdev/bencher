...
async fn run(
    ...
    #[cfg(feature = "plus")]
    shutdown_replica(log, replica_handle, replica_shutdown_timeout).await;
...
#[cfg(feature = "plus")]
async fn shutdown_replica(
    log: &Logger,
    replica_handle: Option<bencher_replica::ReplicatorHandle>,
    replica_shutdown_timeout: Option<Duration>,
) {
    let Some(replica_handle) = replica_handle else {
        return;
    };
    ...
    // The deadline bounds only the ship loop inside the task; a long
    // in-flight tick (snapshot copy) or the sole-mode final checkpoint
    // could otherwise hold this await past Fly's 6 second kill window.
    // On elapse the tail stays in the local WAL and a truncated checkpoint
    // is covered by the engine's crash-safety (lag, never loss), so
    // shutdown continues.
    let outer = deadline + Duration::from_secs(1);
    match tokio::time::timeout(outer, replica_handle.shutdown(deadline)).await {
        Ok(Ok(())) => {},
        Ok(Err(e)) => error!(log, "Replica final sync failed: {e}"),
        Err(_elapsed) => {
            warn!(
                log,
                "Replica final sync exceeded its deadline; continuing shutdown"
            );
        },
    }
}
