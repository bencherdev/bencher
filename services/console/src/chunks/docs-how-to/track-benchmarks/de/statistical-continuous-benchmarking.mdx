...
Anknüpfend an die Tutorials [Schnellstart][quickstart] und [Docker Selbstgehostet][docker self-hosted], fügen wir statistisches [kontinuierliches Benchmarking][continuous benchmarking] zu unserem `claimed` Projekt hinzu.

> 🐰 Stellen Sie sicher, dass Sie
> [einen API-Token erstellt und als die Umgebungsvariable `BENCHER_API_KEY` gesetzt haben][create an api token], bevor Sie fortfahren!
