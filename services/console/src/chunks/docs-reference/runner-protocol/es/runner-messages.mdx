...
| ----------- | ------------------------------------------------- | ----------------------------------------------------------------------- |
| `ready`     | El Runner está inactivo y solicitando un Job      | `poll_timeout` opcional (1-900s) y metadatos del `runner` (`os`, `arch`, `version`, `channel` de actualización opcional, y `checksum` del binario opcional) |
