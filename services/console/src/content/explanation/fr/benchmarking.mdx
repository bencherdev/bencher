...
où la sortie du harnais de benchmark est analysée à l'aide d'un [adaptateur de harnais de benchmark](/fr/docs/explanation/adapters/).
...
S'il existe un [Seuil](/fr/docs/explanation/thresholds/) défini, alors les nouvelles Metrics sont comparées aux Metrics historiques de chaque Benchmark présent dans le Rapport.
...
La seule exception à la mise en garde ci-dessus est l'ignorance d'un Benchmark. Voir [supprimer les alertes](/fr/docs/explanation/thresholds/) pour une vue d'ensemble complète.
...
Les Mesures collectées sont déterminées par l'[adaptateur de harnais de benchmark](/fr/docs/explanation/adapters/).
...
Le Type de Metric est déterminé par l'[adaptateur de harnais de benchmark](/fr/docs/explanation/adapters/).

<br />

## Rapport
Un Rapport est une collection de Benchmarks et de leurs Metrics pour une Branche et une Plateforme de Test spécifiques.
Les Rapports sont le plus souvent générés en utilisant la sous-commande CLI <code><a href="/fr/docs/explanation/bencher-run/">bencher run</a></code>.
Voir [comment suivre les tests de régression de performance](/fr/docs/how-to/track-benchmarks/) pour un aperçu complet.
...
Voir [sélection de branche](/fr/docs/explanation/branch-selection/) pour une vue d'ensemble complète.
...
<h2><a href="/fr/docs/explanation/bencher-run/">Continuez : Sous-commande CLI <code>bencher run</code> ➡</a></h2>