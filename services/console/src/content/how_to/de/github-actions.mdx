...
2. Spezifizieren Sie die Ereignisse, bei denen der Workflow ausgeführt werden soll `on`. Siehe [GitHub Actions `on` Dokumentation](https://docs.github.com/en/actions/using-workflows/workflow-syntax-for-github-actions#on) für eine vollständige Übersicht. Dieser Workflow läuft **nur** bei `push` Ereignissen auf den `main` Branch. Für die Ausführung bei [Pull-Anfragen siehe den Abschnitt unten](#pull-requests).
...
7. Optional: Setzen Sie das `--adapter` Flag oder die `BENCHER_ADAPTER` Umgebungsvariable auf den gewünschten Adapternamen. Wenn dies nicht gesetzt ist, wird der `magic` Adapter verwendet. Siehe [Benchmark-Vergleichsadapter](/docs/explanation/adapters) für eine vollständige Übersicht. (z.B.: `BENCHER_ADAPTER: json`)
...
11. [Verfolgen Sie Ihre Benchmarks](/docs/how-to/track-benchmarks) mit der <code><a href="/docs/explanation/bencher-run">bencher run</a></code> CLI-Unterbefehl:
    1. Es gibt mehrere Optionen zur Einstellung des Projektbranches. Siehe [Branch-Auswahl](/docs/how-to/branch-selection) für eine vollständige Übersicht. Der bereitgestellte Befehl verwendet [GitHub Action Standardumgebungsvariablen](https://docs.github.com/en/actions/learn-github-actions/variables#default-environment-variables) und versucht:
    ...
    2. Stellen Sie den Befehl so ein, dass er einen Fehler ausgibt, wenn ein Alert generiert wird. Damit ein Alert generiert wird, muss bereits eine [Schwelle](/docs/explanation/thresholds) existieren. (z.B.: `--err`)
...
1. Auslösen [am `pull_request_target` Ereignis](https://docs.github.com/en/actions/using-workflows/events-that-trigger-workflows#pull_request_target).
...
1. Kombinieren Sie diesen Workflow mit [dem `workflow_run` Ereignis](https://docs.github.com/en/actions/using-workflows/events-that-trigger-workflows#workflow_run).
...
Siehe [Verwendung von Daten aus dem auslösenden Workflow](https://docs.github.com/en/actions/using-workflows/events-that-trigger-workflows#using-data-from-the-triggering-workflow) für eine vollständige Übersicht.
...
<h2><a href="/docs/explanation/benchmarking">Weitermachen: Übersicht über Benchmarking ➡</a></h2>