...
If the value specified is a slug and the Project does not already exist, it will be created for you when [the `--token` option](#--token-token) is used or no credentials are provided.
...
If the user is authenticated with [the `--token` option](#--token-token), then the Project is added under that user's personal Organization.
