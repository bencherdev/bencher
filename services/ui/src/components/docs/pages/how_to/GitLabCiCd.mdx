...
1. Optional: Set the `--testbed` flag or the `BENCHER_TESTBED` environment variable to the Testbed slug or UUID. If this is not set, then the `localhost` Testbed will be used. (ex: `BENCHER_TESTBED: bullseye`)
...
1. [Track your benchmarks](/docs/how-to/track-benchmarks) with the <code><a href="/docs/reference/bencher-run">bencher run</a></code> CLI subcommand:
...
<h2><a href="/docs/explanation/continuous-benchmarking">Keep Going: Learn about Continuous Benchmarking ➡</a></h2>