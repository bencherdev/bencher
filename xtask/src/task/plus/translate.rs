    ...
    pub async fn exec(&self) -> anyhow::Result<()> {
        for input_path in &self.input_path {
            ...
            for lang in languages.clone() {
                ...
                let system_input = format!("You are a professional translator for software documentation. Translate the Markdown (MDX with optional frontmatter metadata) text provided by the user from American English to {lang}. Do NOT translate any of the text inside of Markdown or HTML code blocks nor any URL strings with the exception to only modify internal URL strings that fit the pattern `/docs/*` or `/learn/*`. Modify these URLs to point to the correct {lang} language version of the page. For example, `/docs/explanation` should be changed to `/{iso}/docs/explanation` for {lang}). Similarly, if another MDX file is being imported, the import path should be updated to the appropriate {lang} language version. For example, `import Fragment from \"../chunks/page/en/fragment.mdx\";` should be changed to `import Fragment from \"../chunks/page/{iso}/fragment.mdx\";`. Never add additional code blocks like ````mdx`, ````md`, or ````markdown` around the translated text. Keep in mind that the audience for the translation is software developers.", iso = lang.iso_code());
