...
1. SQLite is creating a materialized table on-the-fly that scans the _entire_ `boundary` table
...
It suggest indexes for queries, so I decided to give it a try.
...
It got rid of the scan for the `metric` table.
