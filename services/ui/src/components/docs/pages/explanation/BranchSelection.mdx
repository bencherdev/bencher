There are a few ways for setting the project branch in the <code><a href="/docs/reference/bencher-run">bencher run</a></code> CLI subcommand.
...
If both are defined, the `--branch` option takes precedence over the `BENCHER_BRANCH` environement variable.

However, if both of these are missing or the provided value is invalid, then the `bencher run` command will error.
This is not the most convient in CI/CD environments, where new branches are popping up all the time.
...
The `--branch` option conflicts with the `--if-branch` option, but the `BENCHER_BRANCH` environment variable takes precident over the `--if-branch` option.
...
It is also possible to use mutliple `--else-if-branch` options.
...
If the `--if-branch` query succeededs then `--else-branch` is never run, and likewise, if `--if-else-branch` succeeds then `--else-branch` is never run.
